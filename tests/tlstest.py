...
def clientTestCmd(argv):
    ...
    connection.handshakeClientCert(
        serverName=address[0], alpn=[b"http/1.1"], settings=settings
    )
    testConnClient(connection)
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert connection.session.serverName == address[0]
    ...
    synchro.recv(1)
    connection = connect()
    connection.handshakeClientCert(serverName=address[0], alpn=[b"http/1.1"])
    testConnClient(connection)
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert connection.session.serverName == address[0]
    assert connection.session.cipherSuite in constants.CipherSuite.aeadSuites
    ...
    connection.handshakeClientCert(settings=settings)
    testConnClient(connection)
    assert connection.session.cipherSuite in constants.CipherSuite.ecdheEcdsaSuites
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    connection.close()
    ...
    for curve, keySize in (
        ...
        assert isinstance(connection.session.serverCertChain, X509CertChain)
        assert (
            len(connection.session.serverCertChain.getEndEntityPublicKey()) == keySize
        )
        connection.close()
    ...
    testConnClient(connection)
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert len(connection.session.serverCertChain.getEndEntityPublicKey()) == 256
    connection.close()
    ...
    testConnClient(connection)
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert len(connection.session.serverCertChain.getEndEntityPublicKey()) == 384
    connection.close()

    test_no += 1

    print(
        "Test {0} - good X509 RSA and ECDSA, correct RSA and ECDSA sigalgs, RSA, TLSv1.2".format(
            test_no
        )
    )
    synchro.recv(1)
    ...
    testConnClient(connection)
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert connection.session.serverCertChain.getEndEntityPublicKey().key_type == "rsa"
    ...
    print(
        "Test {0} - good X509 RSA and ECDSA, bad RSA and good ECDSA sigalgs, ECDSA, TLSv1.2".format(
            test_no
        )
    )
    synchro.recv(1)
    ...
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert (
        connection.session.serverCertChain.getEndEntityPublicKey().key_type == "ecdsa"
    )
    assert connection.version == (3, 3)
    connection.close()
    ...
    print(
        "Test {0} - good X509 RSA and ECDSA, bad RSA and ECDSA sigalgs, RSA, TLSv1.2".format(
            test_no
        )
    )
    synchro.recv(1)
    ...
    print(
        "Test {0} - good X509 RSA and ECDSA, correct RSA and ECDSA sigalgs, RSA, TLSv1.3".format(
            test_no
        )
    )
    synchro.recv(1)
    ...
    print(
        "Test {0} - good X509 RSA and ECDSA, bad RSA and good ECDSA sigalgs, ECDSA, TLSv1.3".format(
            test_no
        )
    )
    synchro.recv(1)
    ...
    print(
        "Test {0} - good X509 RSA and ECDSA, bad RSA and ECDSA sigalgs, RSA, TLSv1.3".format(
            test_no
        )
    )
    synchro.recv(1)
    ...
    connection.handshakeClientCert(settings=settings)
    testConnClient(connection)
    assert connection.session.cipherSuite in constants.CipherSuite.tls13Suites
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert len(connection.session.serverCertChain.getEndEntityPublicKey()) == 256
    connection.close()
    ...
    assert connection.session.cipherSuite in constants.CipherSuite.tls13Suites
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert len(connection.session.serverCertChain.getEndEntityPublicKey()) == 521
    connection.close()
    ...
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert (
        connection.session.serverCertChain.getEndEntityPublicKey().key_type == "Ed25519"
    )
    connection.close()
    ...
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert (
        connection.session.serverCertChain.getEndEntityPublicKey().key_type == "Ed448"
    )
    connection.close()

    test_no += 1

    print("Test {0} - good RSA and ECDSA, TLSv1.3, rsa".format(test_no))
    synchro.recv(1)
    connection = connect()
    settings = HandshakeSettings()
    settings.minVersion = (3, 4)
    ...
    print("Test {0} - good RSA and ECDSA, TLSv1.3, ecdsa".format(test_no))
    synchro.recv(1)
    connection = connect()
    settings = HandshakeSettings()
    settings.minVersion = (3, 4)
    ...
    print("Test {0} - good RSA and ECDSA, TLSv1.2, rsa".format(test_no))
    synchro.recv(1)
    connection = connect()
    settings = HandshakeSettings()
    settings.minVersion = (3, 3)
    ...
    testConnClient(connection)
    assert (
        connection.session.cipherSuite in constants.CipherSuite.ecdheCertSuites
    ), connection.session.cipherSuite
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert connection.session.serverCertChain.getEndEntityPublicKey().key_type == "rsa"
    assert connection.version == (3, 3)
    connection.close()
    ...
    print("Test {0} - good RSA and ECDSA, TLSv1.2, ecdsa".format(test_no))
    synchro.recv(1)
    connection = connect()
    settings = HandshakeSettings()
    settings.minVersion = (3, 3)
    ...
    testConnClient(connection)
    assert (
        connection.session.cipherSuite in constants.CipherSuite.ecdheEcdsaSuites
    ), connection.session.cipherSuite
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert (
        connection.session.serverCertChain.getEndEntityPublicKey().key_type == "ecdsa"
    ...
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert (
        connection.session.cipherSuite == constants.CipherSuite.TLS_RSA_WITH_RC4_128_MD5
    )
    assert connection.encryptThenMAC == False
    connection.close()
    ...
    if tackpyLoaded:
        ...
        connection.handshakeClientCert(settings=settings)
        assert (
            connection.session.tackExt.tacks[0].getTackId()
            == "5lcbe.eyweo.yxuan.rw6xd.jtoz7"
        )
        ...
        print("Test {0} - good X.509, TACK unrelated to cert chain".format(test_no))
        synchro.recv(1)
        connection = connect()
        try:
            connection.handshakeClientCert(settings=settings)
        ...
        print("Test {0} - good X.509, TACK...skipped (no tackpy)".format(test_no))

        test_no += 1

        print(
            "Test {0} - good X.509, TACK unrelated to cert chain...skipped"
            " (no tackpy)".format(test_no)
        )
    ...
    assert connection.session.cipherSuite in constants.CipherSuite.sha384PrfSuites
    testConnClient(connection)
    connection.close()
    ...
    assert connection.session.serverCertChain is None
    assert connection.ecdhCurve is not None
    assert connection.session.cipherSuite in constants.CipherSuite.sha256PrfSuites
    testConnClient(connection)
    connection.close()
    ...
    connection.handshakeClientCert(settings=settings)
    testConnClient(connection)
    assert connection.session.cipherSuite in constants.CipherSuite.dheDsaSuites
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    connection.close()
    ...
    assert connection.session.cipherSuite in constants.CipherSuite.dheDsaSuites
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert connection.session.serverCertChain.getEndEntityPublicKey().key_type == "dsa"
    connection.close()
    ...
    print("Test {0} - good SRP, prepare to resume... (plus SNI)".format(test_no))
    synchro.recv(1)
    connection = connect()
    settings = HandshakeSettings()
    settings.maxVersion = (3, 3)
    connection.handshakeClientSRP(
        "test", "password", serverName=address[0], settings=settings
    )
    testConnClient(connection)
    connection.close()
    ...
    settings.maxVersion = (3, 3)
    connection.handshakeClientSRP(
        "test", "garbage", serverName=address[0], session=session, settings=settings
    )
    testConnClient(connection)
    ...
    while 1:
        try:
            ...
            for y in range(2):
                ...
                h = HTTPTLSConnection(address[0], address[1], checker=checker)
                for x in range(3):
                    synchro.recv(1)
    ...
    for implementation in implementations:
        for cipher in [
            ...
            if cipher == "3des" and implementation not in ("openssl", "pycrypto"):
                continue
            if cipher in ("aes128gcm", "aes256gcm") and implementation not in (
                "pycrypto",
                "python",
                "openssl",
            ):
            ...
            if cipher in (
                ...
                "aes256ccm",
                "aes256ccm_8",
            ) and implementation not in ("python", "openssl"):
                continue
            if cipher in (
                "chacha20-poly1305_draft00",
                "chacha20-poly1305",
            ...
            if stopTime - startTime:
                print(
                    "100K exchanged at rate of %d bytes/sec"
                    % int(sizeofdata / (stopTime - startTime))
                )
            else:
    ...
    settings.maxVersion = (3, 3)
    connection.handshakeClientCert(
        nextProtos=[b"spdy/2", b"http/1.1"], settings=settings
    )
    # print("  Next-Protocol Negotiated: %s" % connection.next_proto)
    ...
    settings.maxVersion = (3, 3)
    connection.handshakeClientCert(
        nextProtos=[b"spdy/3", b"spdy/2", b"http/1.1"], settings=settings
    )
    # print("  Next-Protocol Negotiated: %s" % connection.next_proto)
    assert connection.next_proto == b"spdy/2"
    connection.close()
    ...
    settings.maxVersion = (3, 3)
    connection.handshakeClientCert(
        serverName=address[0], session=session, settings=settings
    )
    testConnClient(connection)
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert connection.session.serverName == address[0]
    ...
    connection = connect()
    connection.handshakeClientCert(
        serverName=address[0], settings=settings, session=session
    )
    testConnClient(connection)
    assert connection.resumed
    connection.close()
    ...
    print(
        "Test {0} - session_ticket resumption in TLSv1.2 "
    ...
    settings.keyShares = []
    connection.handshakeClientCert(
        x509Chain, x509Key, serverName=address[0], settings=settings
    )
    testConnClient(connection)
    assert isinstance(connection.session.serverCertChain, X509CertChain)
    assert connection.session.serverName == address[0]
    ...
    connection = connect()
    connection.handshakeClientCert(
        x509Chain, x509Key, serverName=address[0], session=session, settings=settings
    )
    testConnClient(connection)
    assert connection.resumed
    connection.close()
    ...
    try:
        ...
        print(
            "Non-critical error: socket error trying to reach internet " "server: ", e
        )
...
def serverTestCmd(argv):
    ...
    with open(os.path.join(dir, "serverRSAPSSKey.pem")) as f:
        x509KeyRSAPSS = parsePEMKey(f.read(), private=True, implementations=["python"])
    ...
    with open(os.path.join(dir, "serverECKey.pem")) as f:
        x509ecdsaKey = parsePEMKey(f.read(), private=True, implementations=["python"])
    ...
    with open(os.path.join(dir, "serverP384ECKey.pem")) as f:
        x509ecdsaP384Key = parsePEMKey(
            f.read(), private=True, implementations=["python"]
        )
    ...
    with open(os.path.join(dir, "serverBrainpoolP256r1ECKey.pem")) as f:
        x509ecdsaBrainpoolP256r1Key = parsePEMKey(
            f.read(), private=True, implementations=["python"]
        )
    ...
    with open(os.path.join(dir, "serverBrainpoolP384r1ECKey.pem")) as f:
        x509ecdsaBrainpoolP384r1Key = parsePEMKey(
            f.read(), private=True, implementations=["python"]
        )
    ...
    with open(os.path.join(dir, "serverBrainpoolP512r1ECKey.pem")) as f:
        x509ecdsaBrainpoolP512r1Key = parsePEMKey(
            f.read(), private=True, implementations=["python"]
        )
    ...
    with open(os.path.join(dir, "serverDSAKey.pem")) as f:
        x509KeyDSA = parsePEMKey(f.read(), private=True, implementations=["python"])
    ...
    with open(os.path.join(dir, "serverEd25519Key.pem")) as f:
        x509Ed25519Key = parsePEMKey(f.read(), private=True, implementations=["python"])
    ...
    with open(os.path.join(dir, "serverEd448Key.pem")) as f:
        x509Ed448Key = parsePEMKey(f.read(), private=True, implementations=["python"])
    ...
    connection = connect()
    connection.handshakeServer(
        certChain=x509Chain, privateKey=x509Key, alpn=[b"http/1.1", b"http/1.0"]
    )
    assert connection.session.serverName == address[0]
    assert connection.extendedMasterSecret
    assert connection.session.appProto == b"http/1.1"
    ...
    connection = connect()
    connection.handshakeServer(
        certChain=x509ChainRSAPSSSig, privateKey=x509KeyRSAPSSSig
    )
    assert connection.extendedMasterSecret
    testConnServer(connection)
    connection.close()
    ...
    synchro.send(b"R")
    connection = connect()
    connection.handshakeServer(certChain=x509ChainRSAPSS, privateKey=x509KeyRSAPSS)
    assert connection.session.serverName == address[0]
    assert connection.extendedMasterSecret
    testConnServer(connection)
    connection.close()
    ...
    settings.maxVersion = (3, 3)
    connection.handshakeServer(
        certChain=x509ChainRSAPSS, privateKey=x509KeyRSAPSS, settings=settings
    )
    assert connection.session.serverName == address[0]
    assert connection.extendedMasterSecret
    testConnServer(connection)
    ...
    settings.record_size_limit = 64
    connection.handshakeServer(
        certChain=x509Chain, privateKey=x509Key, settings=settings
    )
    testConnServer(connection)
    connection.close()
    ...
    settings.maxVersion = (3, 0)
    connection.handshakeServer(
        certChain=x509ecdsaChain, privateKey=x509ecdsaKey, settings=settings
    )
    assert not connection.extendedMasterSecret
    testConnServer(connection)
    connection.close()
    ...
    for curve, certChain, key in (
        ("brainpoolP256r1", x509ecdsaBrainpoolP256r1Chain, x509ecdsaBrainpoolP256r1Key),
        ...
        connection.handshakeServer(
            certChain=certChain, privateKey=key, settings=settings
        )
        assert connection.extendedMasterSecret
    ...
    for curve, exp_chain in (
        ("secp256r1", x509ecdsaChain),
        ...
        settings.virtual_hosts = [v_host]
        connection.handshakeServer(
            certChain=x509ecdsaP384Chain, privateKey=x509ecdsaP384Key, settings=settings
        )
        assert connection.extendedMasterSecret
    ...
    for tls_ver in ("TLSv1.2", "TLSv1,3"):
        print(
            "Test {0} - good X509 RSA and ECDSA, correct RSA and ECDSA sigalgs, RSA, {1}".format(
                test_no, tls_ver
            )
        )
        synchro.send(b"R")
        ...
        settings.virtual_hosts = [v_host]
        connection.handshakeServer(
            certChain=x509ChainRSANonCA, privateKey=x509KeyRSANonCA, settings=settings
        )
        assert connection.extendedMasterSecret
        ...
        print(
            "Test {0} - good X509 RSA and ECDSA, bad RSA and good ECDSA sigalgs, ECDSA, {1}".format(
                test_no, tls_ver
            )
        )
        synchro.send(b"R")
        ...
        print(
            "Test {0} - good X509 RSA and ECDSA, bad RSA and ECDSA sigalgs, RSA, {1}".format(
                test_no, tls_ver
            )
        )
        synchro.send(b"R")
    ...
    settings.maxVersion = (3, 4)
    connection.handshakeServer(
        certChain=x509ecdsaP521Chain, privateKey=x509ecdsaP521Key, settings=settings
    )
    assert connection.extendedMasterSecret
    testConnServer(connection)
    connection.close()
    ...
    settings.maxVersion = (3, 4)
    connection.handshakeServer(
        certChain=x509Ed25519Chain, privateKey=x509Ed25519Key, settings=settings
    )
    assert connection.extendedMasterSecret
    testConnServer(connection)
    connection.close()
    ...
    settings.maxVersion = (3, 4)
    connection.handshakeServer(
        certChain=x509Ed448Chain, privateKey=x509Ed448Key, settings=settings
    )
    assert connection.extendedMasterSecret
    testConnServer(connection)
    connection.close()
    ...
    for prot in ["TLSv1.3", "TLSv1.2"]:
        for c_type, exp_chain in (("rsa", x509Chain), ("ecdsa", x509ecdsaChain)):
            print(
                "Test {0} - good RSA and ECDSA, {2}, {1}".format(test_no, c_type, prot)
            )
            synchro.send(b"R")
            connection = connect()
            settings = HandshakeSettings()
    ...
    if tackpyLoaded:
        tack = Tack.createFromPem(open(os.path.join(dir, "TACK1.pem"), "rU").read())
        ...
        print(
            "Test {0} - good X.509, TACK unrelated to cert chain"
    ...
    connection = connect()
    connection.handshakeServer(
        verifierDB=verifierDB, certChain=x509Chain, privateKey=x509Key
    )
    testConnServer(connection)
    connection.close()
    ...
    connection = connect()
    connection.handshakeServer(
        certChain=x509ecdsaChain, privateKey=x509ecdsaKey, reqCert=True
    )
    testConnServer(connection)
    assert isinstance(connection.session.clientCertChain, X509CertChain)
    assert len(connection.session.clientCertChain.getEndEntityPublicKey()) == 256
    connection.close()
    ...
    connection = connect()
    connection.handshakeServer(
        certChain=x509Ed25519Chain, privateKey=x509Ed25519Key, reqCert=True
    )
    testConnServer(connection)
    assert isinstance(connection.session.clientCertChain, X509CertChain)
    assert (
        connection.session.clientCertChain.getEndEntityPublicKey().key_type == "Ed25519"
    )
    connection.close()
    ...
    settings.maxVersion = (3, 0)
    connection.handshakeServer(
        certChain=x509ChainDSA, privateKey=x509KeyDSA, settings=settings
    )
    assert not connection.extendedMasterSecret
    testConnServer(connection)
    connection.close()
    ...
    settings.maxVersion = (3, 3)
    connection.handshakeServer(
        certChain=x509ChainDSA, reqCert=True, privateKey=x509KeyDSA, settings=settings
    )
    assert isinstance(connection.session.clientCertChain, X509CertChain)
    assert connection.session.clientCertChain.getEndEntityPublicKey().key_type == "dsa"
    testConnServer(connection)
    connection.close()
    ...
    settings.maxVersion = (3, 4)
    connection.handshakeServer(
        certChain=x509Chain, privateKey=x509Key, reqCert=True, settings=settings
    )
    testConnServer(connection)
    ...
    try:
        ...
        assert "Client did not provide a certificate in post-handshake" in str(e)
        assert e.description == AlertDescription.certificate_required
    ...
    for fault in Fault.clientCertFaults + Fault.genericFaults:
        ...
        connection.fault = fault
        connection.handshakeServer(
            certChain=x509Chain, privateKey=x509Key, reqCert=True
        )
        connection.close()
    ...
    class MyHTTPServer(TLSSocketServerMixIn, HTTPServer):
    ...
    connection.handshakeServer(
        ...
        privateKey=x509Key,
        settings=settings,
        nextProtos=[b"spdy/2", b"http/1.1"],
    )
    testConnServer(connection)
    connection.close()
    ...
    connection.handshakeServer(
        ...
        privateKey=x509Key,
        settings=settings,
        nextProtos=[b"http/1.1", b"spdy/2"],
    )
    testConnServer(connection)
    connection.close()
    ...
    connection.handshakeServer(
        ...
        privateKey=x509Key,
        settings=settings,
        nextProtos=[b"http/1.1", b"spdy/2", b"spdy/3"],
    )
    testConnServer(connection)
    connection.close()
    ...
    settings = HandshakeSettings()
    connection.handshakeServer(
        certChain=x509Chain, privateKey=x509Key, settings=settings, nextProtos=[]
    )
    testConnServer(connection)
    connection.close()
    ...
    connection = connect()
    connection.handshakeServer(
        certChain=x509Chain, privateKey=x509Key, sessionCache=sessionCache
    )
    testConnServer(connection)
    connection.close()
