...
from tlslite.utils.cryptomath import secureHMAC, HKDF_expand_label, derive_secret
...
class TestSimple1RTTHandshakeAsClient(unittest.TestCase):
    def test(self):
            ...
            ClientKeyShareExtension().create(
                [
                    KeyShareEntry().create(
                        GroupName.x25519, client_key_public, client_key_private
                    )
                ]
            ),
        ...
        c_hs_traffic = derive_secret(
            secret, bytearray(b"c hs traffic"), handshake_hashes, prf_name
        )
        self.assertEqual(
            c_hs_traffic,
            clean(
        ...
        s_hs_traffic = derive_secret(
            secret, bytearray(b"s hs traffic"), handshake_hashes, prf_name
        )
        self.assertEqual(
            s_hs_traffic,
            clean(
        ...
        server_hs_write_trafic_key = HKDF_expand_label(
            s_hs_traffic, b"key", b"", 16, prf_name
        )
        ...
        server_hs_write_trafic_iv = HKDF_expand_label(
            s_hs_traffic, b"iv", b"", 12, prf_name
        )
        ...
        server_finished_key = HKDF_expand_label(
            s_hs_traffic, b"finished", b"", prf_size, prf_name
        )
        self.assertEqual(
            server_finished_key,
            clean(
        ...
        c_ap_traffic = derive_secret(
            secret, b"c ap traffic", handshake_hashes, prf_name
        )
        ...
        s_ap_traffic = derive_secret(
            secret, b"s ap traffic", handshake_hashes, prf_name
        )
        ...
        server_write_traffic_key = HKDF_expand_label(
            s_ap_traffic, b"key", b"", 16, prf_name
        )
        ...
        server_write_traffic_iv = HKDF_expand_label(
            s_ap_traffic, b"iv", b"", 12, prf_name
        )
        ...
        server_read_hs_key = HKDF_expand_label(c_hs_traffic, b"key", b"", 16, prf_name)
        ...
        server_read_hs_iv = HKDF_expand_label(c_hs_traffic, b"iv", b"", 12, prf_name)
