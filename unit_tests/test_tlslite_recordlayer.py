...
class TestRecordSocket(unittest.TestCase):
    ...
    def test_send_SSLv2_message(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x80"
                + b"\x1f"  # short header
                + b"\x01"  # length - 31 bytes
                + b"\x03\x03"  # CLIENT-HELLO
                + b"\x00\x06"  # TLSv1.2
                + b"\x00\x00"  # cipher suite length
                + b"\x00\x10"  # session_id length
                + b"\x00\x00\x2f"  # Challange length
                + b"\x00\x00\x04"  # cipher: TLS_RSA_WITH_AES_128_CBC_SHA
                + b"\xaa" * 16  # cipher: TLS_RSA_WITH_RC4_128_MD5  # challange
            ),
            mock_sock.sent[0],
        )
    ...
    def test_recv_with_empty_data(self):
        mockSock = MockSocket(
            bytearray(
                b"\x16" + b"\x03\x03" + b"\x00\x00"  # type - handshake  # TLSv1.2
            )
        )  # length
    ...
    def test_recv_with_long_SSL2_header(self):
        mockSock = MockSocket(
            bytearray(
                b"\x40"
                + b"\x04"  # security escape data
                + b"\x00"  # length
                + b"\xaa" * 4  # padding length
            )
        )
...
class TestConnectionState(unittest.TestCase):
    ...
    def test_getSeqNumBytes(self):
        ...
        self.assertEqual(bytearray(b"\x00" * 7 + b"\x01"), connState.getSeqNumBytes())
        self.assertEqual(bytearray(b"\x00" * 7 + b"\x02"), connState.getSeqNumBytes())
        self.assertEqual(bytearray(b"\x00" * 7 + b"\x03"), connState.getSeqNumBytes())
...
class TestRecordLayer(unittest.TestCase):
    ...
    def test_sendRecord_with_encrypting_set_up_tls1_2(self):
        patcher = mock.patch.object(os, "urandom", lambda x: bytearray(x))
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x03"  # application data
                + b"\x00\x30"  # TLS1.2  # length - 48 bytes (3 blocks)
            ),
    ...
    def test_sendRecord_with_encryption_tls1_3_aes_128_gcm(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x03"  # application_data
                + b"\x00\x15"  # hidden protocol version - TLS 1.x  # length
            ),
        )
        self.assertEqual(
            sock.sent[0][5:],
            bytearray(
                b"\xe1\x90\x2d\xd1\xfd" b"u\xdd\xa0\xb1VYB&\xe8\x05\xb1~\xe5u\x9a\x0f"
            ),
        )
    ...
    def test_sendRecord_with_encryption_tls1_3_aes_256_gcm(self):
        ...
        self.assertEqual(
            sock.sent[0][5:],
            bytearray(
                b"}\x17w_#" b"\xfc\\\xaf\x1ef6\x03X\xd2\xe3\x1c\xe4]\xcb\xb7\xbb"
            ),
        )
    ...
    def test_sendRecord_with_encryption_tls1_3_chacha20(self):
        ...
        recordLayer.calcTLS1_3PendingState(
            ciph, bytearray(48), bytearray(48), None  # cl_traffic_sec  # sr_traffic_sec
        )  # implementations
        recordLayer.changeWriteState()
        ...
        self.assertEqual(
            sock.sent[0][5:],
            bytearray(b"o\x9fO\x16\x07" b"\xbdUy\x17E6\n\xd9\x9cT\xec\xdav\x1f\xb4$"),
        )
    ...
    def test_sendRecord_with_padding_tls1_3(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x03"  # application_data
                + b"\x00\x79"  # hidden protocol version - TLS 1.x  # length
            ),
        )
        self.assertEqual(
            sock.sent[0][5:],
    ...
    def test_sendRecord_with_SHA256_tls1_2(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x03"  # application data
                + b"\x00\x40"  # TLS1.2  # length - 64 bytes (4 blocks)
            ),
    ...
    def test_sendRecord_with_encrypting_set_up_tls1_0(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                ...
                + b"\x00\x20"  # TLS1.0  # length - 48 bytes (3 blocks)
            ),
    ...
    def test_sendRecord_with_stream_cipher_and_tls1_0(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x01"  # application data
                + b"\x00\x18"  # SSL3  # length - 24 bytes
            ),
        )  # (4 bytes of data + 20 bytes of MAC)
        self.assertEqual(
            sock.sent[0][5:],
    ...
    def test_sendRecord_with_MD5_MAC_and_tls1_0(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x01"  # application data
                + b"\x00\x14"  # SSL3  # length - 20 bytes
            ),
        )  # (4 bytes of data + 16 bytes of MAC)
        self.assertEqual(
            sock.sent[0][5:],
            bytearray(b"0}R\xe3T\xce`\xf9\x8f\x9d\xe6r\xc4\xdf\xd9\xd5" + b"\xbf/sL"),
        )
    ...
    def test_sendRecord_with_AES256_cipher_and_tls1_0(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x01"  # application data
                + b"\x00\x20"  # SSL3  # length - 32 bytes (2 blocks)
            ),
        )  # (4 bytes of data + 20 bytes of MAC)
        self.assertEqual(
            sock.sent[0][5:],
    ...
    def test_recvRecord_with_AES128GCM_too_short_data(self):
        sock = MockSocket(
            bytearray(
                b"\x17" + b"\x03\x03" + b"\x00\x07" + b"\x00\x00\x00\x00\x00\x00\x00"
            )
        )
    ...
    def test_sendRecord_with_ChaCha20_draft00(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x03"  # application data
                + b"\x00\x14"  # TLSv1.2  # length
            ),
        )
        self.assertEqual(
            sock.sent[0][5:],
            bytearray(b"8k9<\xf3\xdc\x86d,\xf4\xb7\xe8L7\xecA\x7fi\xb1\xdc"),
        )
    ...
    def test_recvRecord_with_ChaCha20_draft00(self):
        sock = MockSocket(
            bytearray(
                ...
                + b"\x03\x03"  # application data
                + b"\x00\x14"  # TLSv1.2
                + b"8k9<\xf3\xdc\x86d,\xf4\xb7\xe8L7\xecA\x7fi\xb1\xdc"  # length
            )
        )
    ...
    def test_sendRecord_with_ChaCha20(self):
        ...
        self.assertEqual(
            sock.sent[0][5:],
            bytearray(b"s\x1e\xe8(+h0\x84\xbf\x96,\xbc\xdf\\\x8c\xad=\x95\xa1\x9a"),
        )
    ...
    def test_recvRecord_with_ChaCha20(self):
        sock = MockSocket(
            bytearray(
                ...
                + b"\x03\x03"  # application data
                + b"\x00\x14"  # TLSv1.2
                + b"s\x1e\xe8(+h0\x84\xbf\x96,\xbc\xdf\\\x8c\xad=\x95\xa1\x9a"  # length
            )
        )
    ...
    def test_sendRecord_with_encrypting_set_up_ssl3(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x00"  # application data
                + b"\x00\x20"  # SSL3  # length - 48 bytes (3 blocks)
            ),
        )  # (4 bytes of data + 20 bytes of MAC)
        self.assertEqual(
            sock.sent[0][5:],
    ...
    def test_if_padding_is_minimal_in_ssl3_high_end(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x17"
                + b"\x03\x00"  # application data
                + b"\x00\x30"  # SSL3  # length - 48 bytes (3 blocks)
            ),
    ...
    def test_sendRecord_with_encryptThenMAC_in_TLSv1_0(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x01"  # application data
                + b"\x00\x24"  # TLS version
                + b"\xc7\xd6\xaf:.MY\x80W\x81\xd2|5A#\xd5"  # length - 1 block + 20 bytes of MAC
    ...
    def test_sendRecord_with_encryptThenMAC_in_TLSv1_2(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x03"  # application data
                + b"\x00\x34"  # TLS version
                + b'H&\x1f\xc1\x9c\xde"\x92\xdd\xe4|\xfco)R\xd6'  # length - IV + 1 block + 20 bytes of MAC
    ...
    def test_recvRecord(self):
        sock = MockSocket(
            bytearray(
                ...
                + b"\x00\x04"  # TLSv1.2
                + b"\x0e"  # length
                + b"\x00\x00\x00"  # server hello done  # length
            )
        )
        recordLayer = RecordLayer(sock)
    ...
    def test_recvRecord_with_invalid_length_payload(self):
        sock = MockSocket(
            bytearray(
                ...
                + b"\x03\x02"  # application data
                + b"\x00\x2f"  # TLSv1.1
                + b"b\x8e\xee\xddV\\W=\x810\xd5\x0c\xae \x84\xa8"  # length
                + b"^\x91\xa4d[\xe4\xde\x90\xee{f\xbb\xcd_\x1ao"
                + b"\xa8\x8c!k\xab\x03\x03\x19.\x1dFMt\x08h"
            )
        )
    ...
    def test_recvRecord_with_zero_length_payload_EtM(self):
        sock = MockSocket(
            bytearray(
                ...
                + b"\x03\x01"  # application data
                + b"\x00\x14"  # TLSv1.0
                + b"A~\x1c\x88s\xdf\xa2sQ\xca\xdd\xb2\xd0\xdc\n\x94\x8e\xc8W\x04"  # length (just MAC alone, no data)
            )
        )
    ...
    def test_recvRecord_with_too_long_application_data_for_record_size_limit(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17" + b"\x03\x03" + b"\x10\x30"  # app data  # SSLv3  # length
            ),
    ...
    def test_recvRecord_with_zero_filled_padding_in_SSLv3(self):
        ...
        def broken_padding(data):
            ...
            paddingBytes = bytearray([0] * (paddingLength)) + bytearray([paddingLength])
            data += paddingBytes
            return data
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x00"  # app data
                + b"\x00\x20"  # SSLv3  # length - 32 bytes
            ),
            sendingSocket.sent[0][:5],
        )
    ...
    def test_recvRecord_with_invalid_last_byte_in_padding(self):
        ...
        def broken_padding(data):
            ...
            paddingBytes = bytearray([paddingLength] * (paddingLength)) + bytearray(
                [255]
            )
            data += paddingBytes
            return data
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x02"  # app data
                + b"\x00\x30"  # tls 1.1  # length - 48 bytes
            ),
            sendingSocket.sent[0][:5],
        )
    ...
    def test_recvRecord_with_invalid_middle_byte_in_padding(self):
        ...
        def broken_padding(data):
            ...
            paddingBytes = bytearray(
                [paddingLength, 0] + [paddingLength] * (paddingLength - 2)
    ...
    def test_recvRecord_with_truncated_MAC(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x02"  # app data
                + b"\x00\x20"  # tls 1.1  # length - 32 bytes
            ),
            sendingSocket.sent[0][:5],
        )
        self.assertEqual(len(sendingSocket.sent[0][5:]), 32)
    ...
    def test_recvRecord_with_encryptThenMAC_and_too_short_MAC(self):
        sock = MockSocket(
            bytearray(
                b"\x17"
                + b"\x03\x03"  # application data
                + b"\x00\x10"  # TLS version
                + b'H&\x1f\xc1\x9c\xde"\x92\xdd\xe4|\xfco)R\xd6'  # length - 16 bytes, less than 20 bytes of MAC
            )
        )
    ...
    def test_recvRecord_with_encryptThenMAC_and_bad_size_encrypted_data(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x02"  # app data
                + b"\x00\x35"  # tls 1.1  # length - IV + data + padding + 1 + hash (20)
            ),
            sendingSocket.sent[0][:5],
        )
    ...
    def test_recvRecord_with_encryptThenMAC_and_bad_last_padding_byte(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x02"  # app data
                + b"\x00\x34"  # tls 1.1  # length - IV + data + padding + SHA-1
            ),
            sendingSocket.sent[0][:5],
        )
    ...
    def test_recvRecord_with_encryptThenMAC_and_SSLv3(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x00"  # app data
                + b"\x00\x24"  # SSLv3  # length - IV + data + padding + SHA-1
            ),
            sendingSocket.sent[0][:5],
        )
    ...
    def test_recvRecord_with_encryptThenMAC_and_bad_padding_byte(self):
        ...
        def broken_padding(data):
            ...
            paddingBytes = (
                bytearray([paddingLength] * (paddingLength - 1))
                + bytearray([0])
                + bytearray([paddingLength])
            )
            data += paddingBytes
    ...
    def test_sendRecord_with_ssl2(self):
        ...
        self.assertEqual(
            sock.sent[2][:2],
            bytearray(b"\x80" + b"\x14"),  # 2 byte header  # overall length
        )
        self.assertEqual(
            sock.sent[2][2:],
            bytearray(b"\xa7\xaai.\x8a\x7ff\x12\xf8T\xcf[)\xc6\xd4\x11\xb85\x13\x0c"),
        )
    ...
    def test_recvRecord_with_ssl2(self):
        ...
        self.assertEqual(
            srv_sock.sent[1][:2], bytearray(b"\x80" + b"\x14")  # 2 byte header
        )  # overall length
        self.assertEqual(
            srv_sock.sent[1][2:],
            bytearray(b"(\x07\xf9\xde`\x80\xa77s\x13Q\xc6%\n\x7f\xbd\xb0,8\xc4"),
        )
