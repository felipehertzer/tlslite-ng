...
class TestTLSConnection(unittest.TestCase):
    ...
    def test_server_with_client_not_using_required_EMS(self):
        ...
        with self.assertRaises(TLSLocalAlert) as err:
            conn.handshakeServer(
                certChain=srv_cert_chain, privateKey=srv_private_key, settings=hs
            )
    ...
    def test_client_with_server_responing_with_wrong_session_id_in_TLS1_3_HRR(self):
        ...
        srv_ext = []
        srv_ext.append(SrvSupportedVersionsExtension().create((3, 4)))
        srv_ext.append(HRRKeyShareExtension().create(GroupName.secp521r1))
    ...
    def test_keyingMaterialExporter_tls1_3_sha384(self):
        ...
        conn.session.cipherSuite = CipherSuite.TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384
        conn.session.exporterMasterSecret = bytearray(
    ...
    def test_keyingMaterialExporter_tls1_3_sha256(self):
        ...
        conn._recordLayer.version = (3, 4)
        conn.session = Session()
        conn.session.cipherSuite = CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256
        conn.session.exporterMasterSecret = bytearray(
    ...
    def test_keyingMaterialExporter_tls1_2_sha384(self):
        ...
        mat = conn.keyingMaterialExporter(bytearray(b"test"), 20)
        self.assertEqual(
            mat, bytearray(b"1\xb8X\xef\x9b\xa5\n9p\x13\xfaxXI\\$\xdf\xb5\xc7i")
        )
    ...
    def test_keyingMaterialExporter_ssl3(self):
        ...
        conn.session = Session()
        conn.session.cipherSuite = CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA
    ...
    def test_keyingMaterialExporter_tls1_2_sha256(self):
        ...
        self.assertEqual(
            mat,
            bytearray(
                b"\xe6EQ\x93\xcb!\xe7\x87\x1e\xdd\x85" + b"\xb2\x08|\xc9\xbfDh\r\x90"
            ),
        )
    ...
    def test_keyingMaterialExporter_tls1_1(self):
        ...
        conn.session = Session()
        conn.session.cipherSuite = CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA

        mat = conn.keyingMaterialExporter(bytearray(b"test"), 20)
        self.assertEqual(
            mat,
            bytearray(b"\x1f\xf8\x18\x01:\x9f\x15a\xd5x\xaa;Y>" + b"\xafG\x92AH\xa4"),
        )
...
class TestRealConnection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def server_process(server):
            settings = HandshakeSettings()
            settings.maxVersion = (3, 3)
            server.handshakeServer(certChain=self.cert_chain, privateKey=self.certKey)
        ...
        self.thread = threading.Thread(target=server_process, args=(self.server,))
        self.thread.start()
