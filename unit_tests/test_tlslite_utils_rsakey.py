...
class TestRSAPSS_components(unittest.TestCase):
    ...
    def test_unknownRSAType(self):
        message = bytearray(
            b"\xad\x8f\xd1\xf7\xf9" + b"\x7fgrRS\xce}\x18\x985" + b"\xb3"
        )
        signed = bytearray(
        ...
        with self.assertRaises(UnknownRSAType):
            self.rsa.hashAndVerify(message, signed, rsaScheme="foo", hAlg="sha1")

    def test_encodingError(self):
        mHash = secureHash(
            bytearray(
                b"\xc7\xf5'\x0f\xcar_\x9b\xd1\x9fQ\x9a\x8d|\xca<"
        ...
        with self.assertRaises(EncodingError):
            self.assertEqual(
                self.rsa.EMSA_PSS_encode(mHash, 10, "sha1", 10),
                bytearray(
                    b"eA=!Fq4\xce\xef5?\xf4\xec\xd8\xa6FPX\xdc~(\xe3"
    ...
    def test_maskTooLong(self):
        with self.assertRaises(MaskTooLongError):
            self.rsa.MGF1(
                bytearray(b"\xad\x8f\xd1\xf7\xf9\x7fgrRS\xce}\x18" + b"\x985\xb3"),
                85899345921,
                "sha1",
            )
    ...
    def test_EMSA_PSS_verifyInvalidSignature29(self):
        with self.assertRaises(InvalidSignature):
            self.rsa.EMSA_PSS_verify(
                bytearray(
                    b"\x6b\x9c\xfa\xc0\xba\x1c\x78\x90\xb1\x3e\x38\x1c"
                ...
                bytearray(
                    b"\x56\x2d\x87\xb5\x78\x1c\x01\xd1\x66\xfe\xf3\x97"
    ...
    def test_EMSA_PSS_verifyInvalidSignature2(self):
        with self.assertRaises(InvalidSignature):
            self.rsa.EMSA_PSS_verify(
                ...
                bytearray(
                    b"eA=!Fq4\xee\xef5?\xf4\xec\xd8\xa6FPX\xdc~(\xe3"
                    + b"\x92\x17z\xa5-\xcfV\xd4)\x99\x8fJ\xb2\x08\xa2<Q"
                    + b"\x02e\xb4\xe0\xecq\xa3:\xe0I\x1f\x83\x9f\xe2"
                    + b"\xf4\xb9\x89\x9b\xdbv\xb8\xb1&\r\xa8\xdfA\x13"
                    + b'\xfd\xed\xce\x85\xb5\x0e\xae\xd0t\xe5`\xbaen"'
    ...
    def test_EMSA_PSS_verifyInvalidSignature3(self):
        with self.assertRaises(InvalidSignature):
            self.rsa.EMSA_PSS_verify(
                ...
                bytearray(
                    b"\xffA=!Fq4\xee\xef5?\xf4\xec\xd8\xa6FPX\xdc~(\xe3"
                    + b"\x92\x17z\xa5-\xcfV\xd4)\x99\x8fJ\xb2\x08\xa2<Q"
                    + b"\x02e\xb4\xe0\xecq\xa3:\xe0I\x1f\x83\x9f\xe2"
                    + b"\xf4\xb9\x89\x9b\xdbv\xb8\xb1&\r\xa8\xdfA\x13"
                    + b'\xfd\xed\xce\x85\xb5\x0e\xae\xd0t\xe5`\xbaen"'
    ...
    def test_EMSA_PSS_verifyInvalidSignature4(self):
        def m(p, hAlg):
            r = secureHash(p, hAlg)
            r[getattr(hashlib, hAlg)().digest_size - 1] = (
                r[getattr(hashlib, hAlg)().digest_size - 1] - 1
            )
            return r
        ...
        with mock.patch("tlslite.utils.rsakey.secureHash", m):
            with self.assertRaises(InvalidSignature):
                self.rsa.EMSA_PSS_verify(
                    bytearray(
                        b"\xc7\xf5'\x0f\xcar_\x9b\xd1\x9fQ\x9a\x8d|"
                    ...
                    bytearray(
                        b"eA=!Fq4\xce\xef5?\xf4\xec\xd8\xa6FPX\xdc~("
    ...
    def test_EMSA_PSS_verifyInvalidSignature5(self):
        ...
        with self.assertRaises(InvalidSignature):
            with mock.patch("tlslite.utils.rsakey.getRandomBytes", m):
                self.rsa.EMSA_PSS_verify(
                    bytearray(
                        b"\xc8\xf5'\x0f\xcar_\x9b\xd1\x9fQ\x9a\x8d|"
                        + b"\xca<\xc5\xc0y\x02@)\xf3\xba\xe5\x10\xf9"
                        + b"\xb0!@\xfe#\x89\x08\xe4\xf6\xc1\x8f\x07\xa8"
                        + b"\x9ch|\x86\x84f\x9b\x1f\x1d\xb2\xba\xf9%"
                        + b"\x1a<\x82\x9f\xac\xcbI0\x84\xe1n\xc9\xe2"
    ...
    def test_EMSA_PSS_verifyInvalidSignature6(self):
        ...
        with self.assertRaises(InvalidSignature):
            with mock.patch("tlslite.utils.rsakey.getRandomBytes", m):
                self.rsa.EMSA_PSS_verify(
                    bytearray(
                        b"\xc7\xf5'\x0f\xcar_\x9b\xd1\x9fQ\x9a"
                    ...
                    bytearray(
                        b"eA=!Fq4\xce\xef5?\xf4\xec\xd8\xa6FPX"
    ...
    def test_EMSA_PSS_encode(self):
        ...
        with mock.patch("tlslite.utils.rsakey.getRandomBytes", m):
            ...
            self.assertEqual(
                self.rsa.EMSA_PSS_encode(mHash, 1023, "sha1", 10),
                bytearray(
                    b"eA=!Fq4\xce\xef5?\xf4\xec\xd8\xa6FPX\xdc~(\xe3"
                    + b"\x92\x17z\xa5-\xcfV\xd4)\x99\x8fJ\xb2\x08\xa2<Q"
                    + b"\x02e\xb4\xe0\xecq\xa3:\xe0I\x1f\x83\x9f\xe2\xf4"
...
class TestRSAPSS_mod1026(unittest.TestCase):
    n = int(
        "2b35541ee2850f34c7727e13eed15d60fb62c0cf230c31bbd0766b6994cff4e5e"
    ...
    e = int("65537", 10)
    d = int(
        "024d35eceb3e11404b7b82d1c6ffea0c77779c33ac7742d2f158cd81f3465c923"
    ...
    p = int(
        "72a5b2ff09d1ad78f1c72e9bc9c0cf1bcc9b03a1595a153886a0273f515e58cc4"
    ...
    q = int(
        "607b3d256cbe2c063105065825c062d2b4495da3362090820b8cf42bb2434e879"
    ...
    qInv = invMod(q, p)
    message = bytearray(
        b"\xc7\xf5\x27\x0f\xca\x72\x5f\x9b\xd1\x9f\x51"
    ...
    def test_RSAPSS_1026(self):
        signed = bytearray(
            b"\x0f\xd2\xef/\xa6\xb6+j\xce\xfc\x16.\xce\x99WH_"
        ...
        self.assertTrue(self.rsa.RSASSA_PSS_verify(mHash, signed, "sha1", 0))
...
class TestRSAPSS_mod1024(unittest.TestCase):
    ...
    n = int(
        "be499b5e7f06c83fa0293e31465c8eb6b58af920bae52a7b5b9bfeb7aa72db126"
    ...
    d = int(
        "0d0f17362bdad181db4e1fe03e8de1a3208989914e14bf269558826bfa20faf4b"
    ...
    p = int(
        "e7a80c5d211c06acb900939495f26d365fc2b4825b75e356f89003eaa5931e6be"
        "5c3f7e6a633ad59db6289d06c354c235e739a1e3f3d39fb40d1ffb9cb44288f",
        16,
    )
    q = int(
        "d248aa248000f720258742da67b711940c8f76e1ecd52b67a6ffe1e49354d66ff"
    ...
    def test_RSAPSS_sha1(self):
        intendedS = bytearray(
            b"\x96\xc3\xf6\x92\x70\x1d\x14\xeb\xbe\xf9"
    ...
    def test_RSAPSS_sha224(self):
        intendedS = bytearray(
            b"\x47\xbd\x25\xf8\x19\xbe\x0f\x7e\xe8\x48\xa3"
    ...
    def test_RSAPSS_sha256(self):
        intendedS = bytearray(
            b"\x11\xe1\x69\xf2\xfd\x40\xb0\x76\x41\xb9\x76"
    ...
    def test_RSAPSS_sha384(self):
        intendedS = bytearray(
            b"\xb2\x81\xad\x93\x4b\x27\x75\xc0\xcb\xa5\xfb"
    ...
    def test_RSAPSS_sha512(self):
        intendedS = bytearray(
            b"\x8f\xfc\x38\xf9\xb8\x20\xef\x6b\x08\x0f\xd2"
    ...
    def test_RSASSA_PSS_verify_sha1(self):
        ...
        mHash = secureHash(self.message, "sha1")
        self.assertTrue(self.rsa.RSASSA_PSS_verify(mHash, signed, "sha1", 10))
    ...
    def test_RSASSA_PSS_verify_shortSign(self):
        with self.assertRaises(InvalidSignature):
            ...
            self.assertTrue(
                self.rsa.RSASSA_PSS_verify(self.message, signed, "sha1", 10)
            )
    ...
    def test_RSASSA_PSS_verify_sha224(self):
        ...
        mHash = secureHash(self.message, "sha224")
        self.assertTrue(self.rsa.RSASSA_PSS_verify(mHash, signed, "sha224", 10))
    ...
    def test_RSASSA_PSS_verify_sha256(self):
        ...
        mHash = secureHash(self.message, "sha256")
        self.assertTrue(self.rsa.RSASSA_PSS_verify(mHash, signed, "sha256", 10))
    ...
    def test_RSASSA_PSS_verify_sha384(self):
        ...
        mHash = secureHash(self.message, "sha384")
        self.assertTrue(self.rsa.RSASSA_PSS_verify(mHash, signed, "sha384", 10))
    ...
    def test_RSASSA_PSS_verify_sha512(self):
        ...
        mHash = secureHash(self.message, "sha512")
        self.assertTrue(self.rsa.RSASSA_PSS_verify(mHash, signed, "sha512", 10))

    def test_RSASSA_PSS_verify_noSalt(self):
        signed = bytearray(
            b"\xafe\x03\xb5\xaf5\x0b\t\xd1?9\x89\xee\x0eP\xcc"
        ...
        mHash = secureHash(self.message, "sha512")
        self.assertTrue(self.rsa.RSASSA_PSS_verify(mHash, signed, "sha512", 0))
...
class TestRSAPSS_mod2048(unittest.TestCase):
    ...
    n = int(
        "c6e0ed537a2d85cf1c4effad6419884d824ceabf5200e755691cb7328acd6a755"
    ...
    d = int(
        "03ac73787e325992a96749d5ef8500e2ccf99e96214dbc22df2c6fde3538aaa85"
    ...
    p = int(
        "e2f7ceb13ea5385ad7659d7bbf0ad4a517c697b70c9d2af7a2193d62b14014412"
    ...
    q = int(
        "e0515147b6e596b7e5140e81365ad698dbeddd874642510f42d357123c20ffb0e"
    ...
    def test_RSAPSS_sha1_messageTooLong(self):
        ...
        def m(x, M, emBits, hAlg, sLen):
            return bytearray(
                b"4c\xa3n\xae\xc9\x15\x84'\xc3&\xd4*A\xb2\x04r"
    ...
    def test_RSAPSS_sha1_noSalt(self):
        ...
        intendedS = bytearray(
            b"6\xbb\x70\x06\x53\x58\x78\xca\x42\x76\x2a"
    ...
    def test_RSAPSS_sha224(self):
        message = bytearray(
            b"\x40\x12\x6e\xcf\x7f\x69\x69\x1e\x10\x74\x4e"
        ...
        intendedS = bytearray(
            b"\x32\xda\x2a\xd3\x9b\x0d\xc4\xd5\x66\xfe\x7d"
    ...
    def test_RSAPSS_sha256(self):
        message = bytearray(
            b"\x81\xea\xf4\x73\xd4\x08\x96\xdb\xf4\xde\xac"
        ...
        intendedS = bytearray(
            b"\x40\xd5\x9e\xbc\x6c\xb7\xb9\x60\xcb\xda\x0d"
    ...
    def test_RSAPSS_sha384(self):
        message = bytearray(
            b"\x32\xa7\xb1\x47\x9a\xcf\x50\x5d\xb7\x93\xf3"
        ...
        intendedS = bytearray(
            b"\x0c\xb3\x75\xec\xc3\x4a\x9f\x36\xb8\x8b\xf5"
    ...
    def test_RSAPSS_sha512(self):
        message = bytearray(
            b"\x35\xa3\x79\x46\xe5\x26\x78\xee\x37\x8f\x5f"
        ...
        intendedS = bytearray(
            b"\x56\x27\x9c\xcd\x2d\x37\xe8\x11\x36\x25\x73"
...
class TestEncryptDecrypt(unittest.TestCase):
    n = int(
        "a8d68acd413c5e195d5ef04e1b4faaf242365cb450196755e92e1215ba59802aa"
    ...
    p = int(
        "c107a2fe924b76e206cb9bc4af2ab7008547c00846bf6d0680b3eac3ebcbd0c7f"
        "d7a54c2b9899b08f80cde1d3691eaaa2816b1eb11822d6be7beaf4e30977c49",
        16,
    )
    q = int(
        "dfea984ce4307eafc0d140c2bb82861e5dbac4f8567cbc981d70440dd63949207"
        "9031486315e305eb83e591c4a2e96064966f7c894c3ca351925b5ce82d8ef0d",
        16,
    )
    d = int(
        "1c23c1cce034ba598f8fd2b7af37f1d30b090f7362aee68e5187adae49b9955c7"
    ...
    def test_encDec(self):
        self.assertEqual(
            bytearray(b"test"), self.rsa.decrypt(self.rsa.encrypt(bytearray(b"test")))
        )
...
class TestRSAPKCS1(unittest.TestCase):
    ...
    def test_hashAndSign_RSAPKCS1_sha1(self):
        sigBytes = self.rsa.hashAndSign(self.message, "PKCS1", "sha1")
        self.assertEqual(
            sigBytes,
            bytearray(
    ...
    def test_hashAndSign_wrongRSaAlgorithm(self):
        with self.assertRaises(UnknownRSAType):
            self.rsa.hashAndSign(self.message, "PKC1", "sha1")

    def test_hashAndSign_RSAPKCS1_sha1_notSet(self):
        sigBytes = self.rsa.hashAndSign(self.message, "PKCS1")
        self.assertEqual(
            sigBytes,
            bytearray(
                b"\x17\x50\x15\xbd\xa5\x0a\xbe\x0f\xa7\xd3\x9a\x83\x53\x88"
    ...
    def test_hashAndSign_RSAPKCS1_sha224(self):
        sigBytes = self.rsa.hashAndSign(self.message, "PKCS1", "sha224")
        self.assertEqual(
            sigBytes,
            bytearray(
    ...
    def test_hashAndSign_RSAPKCS1_sha256(self):
        sigBytes = self.rsa.hashAndSign(self.message, "PKCS1", "sha256")
        self.assertEqual(
            sigBytes,
            bytearray(
    ...
    def test_hashAndSign_RSAPKCS1_sha384(self):
        sigBytes = self.rsa.hashAndSign(self.message, "PKCS1", "sha384")
        self.assertEqual(
            sigBytes,
            bytearray(
    ...
    def test_hashAndSign_RSAPKCS1_sha512(self):
        sigBytes = self.rsa.hashAndSign(self.message, "PKCS1", "sha512")
        self.assertEqual(
            sigBytes,
            bytearray(
    ...
    def test_hashAndVerify_PKCS1_sha1_notSet(self):
        ...
        self.assertTrue(self.rsa.hashAndVerify(sigBytes, self.message, "PKCS1"))
    ...
    def test_hashAndVerify_PKCS1_sha224(self):
        ...
        self.assertTrue(
            self.rsa.hashAndVerify(sigBytes, self.message, "PKCS1", "sha224")
        )
    ...
    def test_hashAndVerify_PKCS1_sha256(self):
        ...
        self.assertTrue(
            self.rsa.hashAndVerify(sigBytes, self.message, "PKCS1", "sha256")
        )
    ...
    def test_hashAndVerify_PKCS1_sha384(self):
        ...
        self.assertTrue(
            self.rsa.hashAndVerify(sigBytes, self.message, "PKCS1", "sha384")
        )
    ...
    def test_hashAndVerify_PKCS1_sha512(self):
        ...
        self.assertTrue(
            self.rsa.hashAndVerify(sigBytes, self.message, "PKCS1", "sha512")
        )
    ...
    def test_verify_PKCS1_sha512(self):
        ...
        self.assertTrue(
            self.rsa.verify(
                sigBytes, secureHash(self.message, "sha512"), hashAlg="sha512"
            )
        )
...
class TestRSAKey(unittest.TestCase):
    ...
    N = int(
        "101394340507163232476731540998223559348384567842249950630680016"
        "729829651735259973644737329194901739140557378171784099933376993"
        "53519793819698299093375577631"
    )
    e = 65537
    d = int(
        "141745721972918790698280063566067268498148845185400775263435953"
        "111621933337897734637889622802200979017278309730638712431978569"
        "771023240787627463565420833"
    )
    p = int(
        "903614668974112441151570413608036278756730123846327797584414732"
        "71561046135679"
    )
    q = int(
        "112209710608480690748363491355148749700390327497055102381924341"
        "581861552321889"
    )
    dP = int(
        "37883511062045429960298073888481933556799848761465588242411735"
        "654811958185817"
    )
    dQ = int(
        "62620473256245674709410658602365234471246407950887183034263101"
        "286525236349249"
    )
    qInv = int(
        "479278327226690415958629934820002183615697717603796111150941"
        "44623120451328875"
    )
    ...
    def test_hashAndSign_PSS(self):
        ...
        self.assertEqual(
            bytearray(
                b"op\xfa\x1d\xfa\xe8i\xf2zV\x9a\xf4\x8d"
    ...
    def test_hashAndVerify_PSS(self):
        ...
        self.assertTrue(
            rsa.hashAndVerify(sigBytes, bytearray(b"text to sign"), "PSS", "sha1")
        )
    ...
    def test_verify_PSS(self):
        ...
        self.assertTrue(
            rsa.verify(
                sigBytes,
                secureHash(bytearray(b"text to sign"), "sha1"),
                "pss",
                "sha1",
                0,
            )
    ...
    def test_hashAndVerify_with_invalid_signature(self):
        ...
        self.assertFalse(rsa.hashAndVerify(sigBytes, bytearray(b"text to sign")))
    ...
    def test_addPKCS1SHA1Prefix(self):
        ...
        self.assertEqual(
            RSAKey.addPKCS1SHA1Prefix(data),
            bytearray(
    ...
    def test_addPKCS1SHA1Prefix_without_NULL(self):
        ...
        self.assertEqual(
            RSAKey.addPKCS1SHA1Prefix(data, False),
            bytearray(
                b"0\x1f0\x07\x06\x05+\x0e\x03\x02\x1a\x04\x14" + b" sha-1 hash of data "
            ),
        )
    ...
    def test_addPKCS1Prefix(self):
        ...
        self.assertEqual(
            RSAKey.addPKCS1Prefix(data, "sha1"),
            bytearray(
                b"0!0\t\x06\x05+\x0e\x03\x02\x1a\x05\x00\x04\x14"
                + b" sha-1 hash of data "
            ),
...
"""
    ...
    def test_simple_encypt_decrypt(self):
        ...
        # message back
        self.assertEqual(
            self.priv_key.decrypt(self.pub_key.encrypt(b"message")), b"message"
        )
...
1ee1e7f8843b7d16f9f37ef35748c3ae93aa155cdcdfeb4e78567303"""
        ...
        # sanity check that the decrypted ciphertext is invalid
        dec = self.priv_key._raw_private_key_op_bytes(ciphertext)
        self.assertEqual(dec[0:11], b"\x78\x05\x5c\xc0\xd7\x02\xfe\xd7\x6a\xbe\x53")
...
-----END PUBLIC KEY-----"""
    ...
    def test_simple_with_empty(self):
        self.assertEqual(b"", self.priv_key.decrypt(self.pub_key.encrypt(b"")))
