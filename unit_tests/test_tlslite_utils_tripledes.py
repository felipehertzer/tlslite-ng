...
class Test3DES_components(unittest.TestCase):
    ...
    def test_too_short_key_size(self):
        with self.assertRaises(ValueError):
            Python_TripleDES(
                b"\x7c\xa1\x10\x45\x4a\x1a\x6e\x57", b"\x55\xfe\x07\x2a\x73\x51\xa5\x00"
            )

    @unittest.skipIf(
        PY_VER < (3,), "DeprecationWarning check should apply only on python3"
    )
    ...
    def test_1des_too_short_key(self):
        with self.assertRaises(ValueError):
            Des(b"\x00\x00\x00\x00\x00\x00\x00", b"\x00\x00\x00\x00\x00\x00\x00\x00")

    def test_3des_16B_key(self):
        key = bytearray(
            b"\x7c\xa1\x10\x45\x4a\x1a\x6e\x57" b"\x7c\xa1\x10\x45\x4a\x1a\x6e\x57"
        )
...
class Test3DES_KATs_KO3(unittest.TestCase):
    ...
    def test_3des_vartext_encrypt(self):
        ...
        self.assertEqual(ret, b"\x95\xf8\xa5\xe5\xdd\x31\xd9\x00")
        self.assertIsInstance(ret, bytearray)
    ...
    def test_3des_vartext_decrypt(self):
        ...
        self.assertEqual(ret, b"\x80\x00\x00\x00\x00\x00\x00\x00")
        self.assertIsInstance(ret, bytearray)
    ...
    def test_3des_varkey_encrypt(self):
        ...
        key = bytearray(
            b"\x80\x01\x01\x01\x01\x01\x01\x01"
            b"\x01\x01\x01\x01\x01\x01\x01\x01"
            b"\x01\x01\x01\x01\x01\x01\x01\x01"
        )
        iv = b"\x00\x00\x00\x00\x00\x00\x00\x00"
