...
class TestMessageSocket(unittest.TestCase):
    ...
    def test_recvMessage(self):
        ...
        sock = MockSocket(
            bytearray(
                b"\x15"
                + b"\x03\x03"  # message type
                + b"\x00\x04"  # TLS version
                + b"\xff\xff"  # payload length
                + b"\xbb\xbb"  # first message  # second message
            )
        )
    ...
    def test_recvMessage_with_blocking_socket(self):
        ...
        sock = MockSocket(
            bytearray(
                b"\x15"
                + b"\x03\x03"  # message type
                + b"\x00\x02"  # TLS version
                + b"\xff\xff"  # payload length  # message
            ),
    ...
    def test_queueMessage(self):
        ...
        self.assertEqual(
            sock.sent[0],
            bytearray(b"\x15" + b"\x00\x00" + b"\x00\x04" + b"\xff\xbb" + b"\xff\xaa"),
        )
    ...
    def test_queueMessage_with_conflicting_types(self):
        ...
        self.assertEqual(len(sock.sent), 1)
        self.assertEqual(
            bytearray(b"\x16" + b"\x03\x03" + b"\x00\x03" + b"\xaa" * 3), sock.sent[0]
        )
        ...
        self.assertEqual(
            bytearray(b"\x15" + b"\x03\x03" + b"\x00\x02" + b"\x02\x01"), sock.sent[1]
        )
