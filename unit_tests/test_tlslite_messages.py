...
class TestHelloMessage(unittest.TestCase):
    ...
    def test_addExtension(self):
        ...
        self.assertEqual(
            msg.extensions, [SNIExtension().create(bytearray(b"example.com"))]
        )
...
class TestClientHello(unittest.TestCase):
    ...
    def test_create_with_one_ciphersuite(self):
        ...
        self.assertEqual(bytearray(0), client_hello.session_id)
        self.assertEqual(
            [CipherSuite.TLS_EMPTY_RENEGOTIATION_INFO_SCSV], client_hello.cipher_suites
        )
        self.assertEqual([0], client_hello.compression_methods)
    ...
    def test_create_with_random(self):
        client_hello = ClientHello()
        client_hello.create(
            (3, 0), bytearray(b"\x01" + b"\x00" * 30 + b"\x02"), bytearray(0), []
        )

        self.assertEqual((3, 0), client_hello.client_version)
        self.assertEqual(
            bytearray(b"\x01" + b"\x00" * 30 + b"\x02"), client_hello.random
        )
        self.assertEqual(bytearray(0), client_hello.session_id)
        self.assertEqual([], client_hello.cipher_suites)
        self.assertEqual([0], client_hello.compression_methods)
    ...
    def test_create_with_NPN_in_extensions(self):
        client_hello = ClientHello()
        client_hello.create(
            (3, 0), bytearray(32), bytearray(0), [1], extensions=[NPNExtension()]
        )
    ...
    def test_parse(self):
        p = Parser(
            bytearray(
                ...
                b"\x00" * 2
                + b"\x26"
                + b"\x01\x01"  # length - 38 bytes
                + b"\x00" * 32  # protocol version - arbitrary (invalid)
                + b"\x00"  # client random
                + b"\x00" * 2  # session ID length
                + b"\x00"  # cipher suites length  # compression methods length
            )
        )
        client_hello = ClientHello()
    ...
    def test_parse_with_empty_extensions(self):
        p = Parser(
            bytearray(
                ...
                + b"\x00"  # client random
                + b"\x00" * 2  # session ID length
                + b"\x00"  # cipher suites length
                + b"\x00\x00"  # compression methods length  # extensions length
            )
        )
        client_hello = ClientHello()
        client_hello = client_hello.parse(p)
    ...
    def test_parse_with_SNI_extension(self):
        p = Parser(
            bytearray(
                ...
                b"\x00" * 2
                + b"\x3c"
                + b"\x01\x01"  # length - 60 bytes
                + b"\x00" * 32  # protocol version - arbitrary (invalid)
                + b"\x00"  # client random
                + b"\x00" * 2  # session ID length
                + b"\x00"  # cipher suites length
                + b"\x00\x14"  # compression methods length
                + b"\x00\x00"  # extensions length - 20 bytes
                + b"\x00\x10"  # extension type - SNI (0)
                + b"\x00\x0e"  # extension length - 16 bytes
    ...
    def test_parse_with_cert_type_extension(self):
        p = Parser(
            bytearray(
                ...
                b"\x00" * 2
                + b"\x2f"
                + b"\x01\x01"  # length - 47 bytes
                + b"\x00" * 32  # protocol version - arbitrary (invalid)
                + b"\x00"  # client random
                + b"\x00" * 2  # session ID length
                + b"\x00"  # cipher suites length
                + b"\x00\x07"  # compression methods length
                + b"\x00\x09"  # extensions length - 7 bytes
                + b"\x00\x03"  # extension type - certTypes (9)
                + b"\x02"  # extension length - 3 bytes
                + b"\x00"  # length of array - 2 bytes
                + b"\x01"  # type - x509 (0)  # type - opengpg (1)
            )
        )
        client_hello = ClientHello()
        client_hello = client_hello.parse(p)
    ...
    def test_parse_with_SRP_extension(self):
        p = Parser(
            bytearray(
                ...
                b"\x00" * 2
                + b"\x35"
                + b"\x01\x01"  # length - 53 bytes
                + b"\x00" * 32  # protocol version - arbitrary (invalid)
                + b"\x00"  # client random
                + b"\x00" * 2  # session ID length
                + b"\x00"  # cipher suites length
                + b"\x00\x0d"  # compression methods length
                + b"\x00\x0c"  # extensions length - 13 bytes
                + b"\x00\x09"  # extension type - SRP (12)
                + b"\x08"  # extension length - 9 bytes
                + b"username"  # length of name - 8 bytes  # UTF-8 encoding of "username" :)
            )
        )
        client_hello = ClientHello()
        client_hello = client_hello.parse(p)
    ...
    def test_parse_with_NPN_extension(self):
        p = Parser(
            bytearray(
                ...
                b"\x00" * 2
                + b"\x2c"
                + b"\x01\x01"  # length - 44 bytes
                + b"\x00" * 32  # protocol version - arbitrary (invalid)
                + b"\x00"  # client random
                + b"\x00" * 2  # session ID length
                + b"\x00"  # cipher suites length
                + b"\x00\x04"  # compression methods length
                + b"\x33\x74"  # extensions length - 4 bytes
                + b"\x00\x00"  # extension type - NPN (13172)  # extension length - 0 bytes
            )
        )
        client_hello = ClientHello()
        client_hello = client_hello.parse(p)
    ...
    def test_parse_with_TACK_extension(self):
        p = Parser(
            bytearray(
                ...
                + b"\x00"  # cipher suites length
                + b"\x00\x04"  # compression methods length
                + b"\xf3\x00"  # extensions length - 4 bytes
                + b"\x00\x00"  # extension type - TACK (62208)  # extension length - 0 bytes
            )
        )
        client_hello = ClientHello()
        client_hello = client_hello.parse(p)
    ...
    def test_write(self):
        ...
        self.assertEqual(
            list(
                bytearray(
                    b"\x01"
                    + b"\x00" * 2  # type of message - client_hello
                    + b"\x26"
                    + b"\x00" * 2  # length - 38 bytes
                    + b"\x00" * 32  # protocol version
                    + b"\x00"  # client random
                    + b"\x00" * 2  # session ID length
                    + b"\x00"  # cipher suites length  # compression methods length
                )
        ...
        self.assertEqual(
            list(
                bytearray(
                    ...
                    + b"\x00" * 2  # type of message - client_hello
                    + b"\x30"
                    + b"\x03\x01"  # length - 48 bytes
                    + b"\x00" * 31  # protocol version (TLS 1.0)
                    + b"\xff"
                    + b"\x00"  # client random
                    + b"\x00\x00"  # session ID length
                    + b"\x01"  # cipher suites length
                    + b"\x00"  # compression methods length
                    + b"\x00\x07"  # supported method - NULL
                    + b"\x00\x09"  # extensions length
                    + b"\x00\x03"  # cert_type extension value (9)
                    + b"\x02"  # size of the extension
                    + b"\x00"  # length of supported types
                    + b"\x01"  # type - X.509  # type - OpenPGP
                )
            ),
            list(client_hello.write()),
        )
    ...
    def test_invalid_srp_username(self):
        with self.assertRaises(TypeError):
            ClientHello().create(
                (3, 1), bytearray(b"\x00" * 32), bytearray(0), [], srpUsername="test"
            )
        ...
        self.assertEqual(
            list(
                bytearray(
                    ...
                    + b"\x01"  # cipher suites length
                    + b"\x00"  # compression methods length
                    + b"\x00\x11"  # supported method - NULL
                    + b"\x00\x0c"  # extensions length
                    + b"\x00\x0d"  # srp extension value (12)
                    + b"\x0c"  # size of the extension
                    +  # length of name
    ...
    def test_write_with_tack(self):
        client_hello = ClientHello().create(
            (3, 1), bytearray(b"\x00" * 31 + b"\xff"), bytearray(0), [], tack=True
        )
        ...
        self.assertEqual(
            list(
                bytearray(
                    ...
                    + b"\x00" * 2  # type of message - client_hello
                    + b"\x2d"
                    + b"\x03\x01"  # length - 45 bytes
                    + b"\x00" * 31  # protocol version
                    + b"\xff"
                    + b"\x00"  # client random
                    + b"\x00\x00"  # session ID length
                    + b"\x01"  # cipher suites length
                    + b"\x00"  # compression methods length
                    + b"\x00\x04"  # supported method - NULL
                    + b"\xf3\x00"  # extensions length
                    + b"\x00\x00"  # TACK extension value (62208)  # size of the extension
                )
            ),
            list(client_hello.write()),
        )
        ...
        self.assertEqual(
            list(
                bytearray(
                    ...
                    + b"\x00"  # compression methods length
                    + b"\x00\x04"  # supported method - NULL
                    + b"\x33\x74"  # extensions length
                    + b"\x00\x00"  # NPN extension value (13172)  # size of the extension
                )
            ),
            list(client_hello.write()),
        )
        ...
        self.assertEqual(
            list(
                bytearray(
                    ...
                    + b"\x01"  # cipher suites length
                    + b"\x00"  # compression methods length
                    + b"\x00\x14"  # supported method - NULL
                    + b"\x00\x00"  # extensions length
                    + b"\x00\x10"  # servername extension value (0)
                    + b"\x00\x0e"  # byte size of the extension
                    + b"\x00"  # length of the list
                    + b"\x00\x0b"  # name type: host_name (0)
                    +  # length of host name
                    # utf-8 encoding of "example.com"
                    b"\x65\x78\x61\x6d\x70\x6c\x65\x2e\x63\x6f\x6d"
                )
    ...
    def test___str__(self):
        ...
        self.assertEqual(
            "client_hello,version(3.0),random(...),"
    ...
    def test___str___with_unassigned_cipher_suite(self):
        client_hello = ClientHello().create((3, 0), bytearray(4), bytearray(0), [93])
    ...
    def test___repr__(self):
        ...
        self.assertEqual(
            "ClientHello(ssl2=False, client_version=(3.3), "
    ...
    def test_server_name_other_than_dns_name(self):
        ...
        sni_ext = SNIExtension().create(
            serverNames=[SNIExtension.ServerName(1, b"test")]
        )
    ...
    def test_parse_with_SSLv2_client_hello_and_short_random(self):
        parser = Parser(
            bytearray(
                ...
                b"\x00\x02"
                + b"\x00\x15"  # version - SSLv2
                + b"\x00\x00"  # cipher spec length - 21 bytes
                + b"\x00\x10"  # session ID length - 0 bytes
                + b"\x07\x00\xc0"  # challange length - 16 bytes
                + b"\x05\x00\x80"  # cipher - SSL2_DES_192_EDE3_CBC_WITH_MD5
                + b"\x03\x00\x80"  # cipher - SSL2_IDEA_128_CBC_WITH_MD5
                + b"\x01\x00\x80"  # cipher - SSL2_RC2_CBC_128_CBC_WITH_MD5
                + b"\x06\x00\x40"  # cipher - SSL2_RC4_128_WITH_MD5
                + b"\x04\x00\x80"  # cipher - SSL2_DES_64_CBC_WITH_MD5
                + b"\x02\x00\x80"  # cipher - SSL2_RC2_CBC_128_CBC_WITH_MD5
                + b"\x01" * 16  # cipher - SSL2_RC4_128_EXPORT40_WITH_MD5  # challenge
            )
        )
        ...
        self.assertEqual(bytearray(b"\x00" * 16 + b"\x01" * 16), client_hello.random)
        self.assertEqual([0], client_hello.compression_methods)
    ...
    def test_parse_with_SSLv2_client_hello(self):
        parser = Parser(
            bytearray(
                ...
                # b'\x01' +               # message type - client hello
                b"\x03\x02"
                + b"\x00\x06"  # version - TLSv1.1
                + b"\x00\x10"  # cipher spec length - 6 bytes
                + b"\x00\x20"  # session ID length - 16 bytes
                + b"\x07\x00\xc0"  # challange length - 32 bytes
                + b"\x00\x00\x2f"  # cipher - SSL2_DES_192_EDE3_CBC_WITH_MD5
                + b"\xff" * 16  # cipher - TLS_RSA_WITH_AES_128_CBC_SHA
                + b"\x01" * 32  # session_id  # challenge
            )
        )
        client_hello = ClientHello(ssl2=True)
        ...
        self.assertEqual(bytearray(b"\x01" * 32), client_hello.random)
        self.assertEqual([0], client_hello.compression_methods)

    def test_write_with_SSLv2(self):
        client_hello = ClientHello(ssl2=True)
        ciphers = [0x0700C0, CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA]

        client_hello.create(
            (3, 2),
            random=bytearray(b"\xab" * 16),
        ...
        self.assertEqual(
            bytearray(
                b"\x01"
                + b"\x03\x02"  # type of message - CLIENT HELLO
                + b"\x00\x06"  # version - TLSv1.1
                + b"\x00\x00"  # cipher list length
                + b"\x00\x10"  # session id length
                + b"\x07\x00\xc0"  # challange length
                + b"\x00\x00\x2f"  # cipher - SSL2_DES_192_EDE3_CBC_WITH_MD5
                + b"\xab" * 16  # cipher - TLS_RSA_WITH_AES_128_CBC_SHA
            ),  # challange
            client_hello.write(),
        )
    ...
    def test_parse_with_short_message(self):
        p = Parser(
            bytearray(
                ...
                + b"\x00" * 32  # version
                + b"\x00"  # client random
                + b"\x00\x00"  # session id length  # cipher suites length
...
class TestServerHello(unittest.TestCase):
    ...
    def test_next_protos_reset(self):
        ...
        server_hello.next_protos = [bytearray(b"spdy/3"), bytearray(b"http/1.1")]
        self.assertEqual(
            [bytearray(b"spdy/3"), bytearray(b"http/1.1")], server_hello.next_protos
        )
    ...
    def test_create_with_minimal_options(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x02"
                + b"\x00\x00\x2a"  # type of message
                + b"\x03\x03"  # length
                + b"\x02" * 31  # server version
                + b"\x01"
                + b"\x04"  # random
                + b"\x00" * 4  # Session ID length
                + b"\x00\x04"  # session id
                + b"\x00"  # selected ciphersuite  # selected compression method
            ),
            server_hello.write(),
        )
    ...
    def test_parse(self):
        p = Parser(
            bytearray(
                ...
                b"\x00\x00\x36"
                + b"\x03\x03"  # length - 54 bytes
                + b"\x01" * 31  # version - TLS 1.2
                + b"\x02"
                + b"\x00"  # random
                + b"\x00\x9d"  # session id length
                + b"\x01"  # cipher suite
                + b"\x00\x0e"  # compression method (zlib)
                + b"\xff\x01"  # extensions length - 14 bytes
                + b"\x00\x01"  # ext type - renegotiation_info
                + b"\x00"  # ext length - 1 byte
                + b"\x00\x23"  # value - supported (0)
                + b"\x00\x00"  # ext type - session ticket (35)
                + b"\x00\x0f"  # ext length - 0 bytes
                + b"\x00\x01"  # ext type - heartbeat (15)
                + b"\x01"  # ext length - 1 byte
            )
        )  # peer allowed to send requests (1)
        server_hello = ServerHello()
    ...
    def test_parse_with_length_short_by_one(self):
        p = Parser(
            bytearray(
                ...
                # b'\x02' +                     # type of message - server_hello
                b"\x00\x00\x25"
                + b"\x03\x03"  # length - 37 bytes (one short)
                + b"\x01" * 31  # version - TLS 1.2
                + b"\x02"
                + b"\x00"  # random
                + b"\x00\x9d"  # session id length
                + b"\x01"  # cipher suite  # compression method (zlib)
            )
        )
        server_hello = ServerHello()
    ...
    def test_parse_with_length_long_by_one(self):
        p = Parser(
            bytearray(
                ...
                # b'\x02' +                     # type of message - server_hello
                b"\x00\x00\x27"
                + b"\x03\x03"  # length - 39 bytes (one long)
                + b"\x01" * 31  # version - TLS 1.2
                + b"\x02"
                + b"\x00"  # random
                + b"\x00\x9d"  # session id length
    ...
    def test_parse_with_extensions_length_short_by_one(self):
        p = Parser(
            bytearray(
                ...
                + b"\x00\x9d"  # session id length
                + b"\x01"  # cipher suite
                + b"\x00\x0d"  # compression method (zlib)
                + b"\xff\x01"  # extensions length - 13 bytes (!)
                + b"\x00\x01"  # ext type - renegotiation_info
                + b"\x00"  # ext length - 1 byte
                + b"\x00\x23"  # value - supported (0)
                + b"\x00\x00"  # ext type - session ticket (35)
    ...
    def test_parse_with_extensions_length_long_by_one(self):
        p = Parser(
            bytearray(
                ...
                + b"\x00\x9d"  # session id length
                + b"\x01"  # cipher suite
                + b"\x00\x0f"  # compression method (zlib)
                + b"\xff\x01"  # extensions length - 15 bytes (!)
                + b"\x00\x01"  # ext type - renegotiation_info
                + b"\x00"  # ext length - 1 byte
                + b"\x00\x23"  # value - supported (0)
                + b"\x00\x00"  # ext type - session ticket (35)
    ...
    def test_parse_with_cert_type_extension(self):
        p = Parser(
            bytearray(
                b"\x00\x00\x2d"
                + b"\x03\x03"  # length - 45 bytes
                + b"\x01" * 31  # version - TLS 1.2
                + b"\x02"
                + b"\x00"  # random
                + b"\x00\x9d"  # session id length
                + b"\x00"  # cipher suite
                + b"\x00\x05"  # compression method (none)
                + b"\x00\x09"  # extensions length - 5 bytes
                + b"\x00\x01"  # ext type - cert_type (9)
                + b"\x01"  # ext length - 1 byte  # value - OpenPGP (1)
            )
        )
    ...
    def test_parse_with_no_extensions(self):
        p = Parser(
            bytearray(
                ...
                + b"\x00"  # random
                + b"\x00\x9d"  # session id length
                + b"\x00"  # cipher suite  # compression method (none)
            )
        )
    ...
    def test_parse_with_bad_cert_type_extension(self):
        p = Parser(
            bytearray(
                b"\x00\x00\x2e"
                + b"\x03\x03"  # length - 46 bytes
                + b"\x01" * 31  # version - TLS 1.2
                + b"\x02"
                + b"\x00"  # random
                + b"\x00\x9d"  # session id length
                + b"\x00"  # cipher suite
                + b"\x00\x06"  # compression method (none)
                + b"\x00\x09"  # extensions length - 5 bytes
                + b"\x00\x02"  # ext type - cert_type (9)
                + b"\x00\x01"  # ext length - 2 bytes  # value - X.509 (0), OpenPGP (1)
            )
        )
    ...
    def test_parse_with_NPN_extension(self):
        p = Parser(
            bytearray(
                b"\x00\x00\x3c"
                + b"\x03\x03"  # length - 60 bytes
                + b"\x01" * 31  # version - TLS 1.2
                + b"\x02"
                + b"\x00"  # random
                + b"\x00\x9d"  # session id length
                + b"\x00"  # cipher suite
                + b"\x00\x14"  # compression method (none)
                + b"\x33\x74"  # extensions length - 20 bytes
                + b"\x00\x10"  # ext type - npn
                + b"\x08"  # ext length - 16 bytes
                + b"http/1.1"  # length of first name - 8 bytes
                + b"\x06"
                + b"spdy/3"  # length of second name - 6 bytes
            )
        )
    ...
    def test_parse_tls1_3(self):
        parser = Parser(
            bytearray(
                ...
                b"\x00\x00\x2c"
                + b"\x03\x04"  # overall length
                + b"\x02" * 32  # protocol version
                + b"\x04"  # random
                + b"\x03" * 4  # session_id len
                + b"\x00\x04"  # session_id
                + b"\x00"  # cipher suite
    ...
    def test_parse_tls1_3_hrr(self):
        parser = Parser(
            bytearray(
                b"\x00\x00\x30"
                + b"\x03\x04"  # overall length
                + TLS_1_3_HRR  # protocol version
                + b"\x04"  # random
                + b"\x03" * 4  # session_id len
                + b"\x00\x04"  # session_id
                + b"\x00"  # cipher suite
                + b"\x00\x04"  # compression method
                + b"\x00\xff"  # extensions
                + b"\x00\x00"  # extID
            )
        )  # ext length
        ...
        self.assertEqual(server_hello.compression_method, 0)
        self.assertEqual(
            server_hello.extensions, [TLSExtension(extType=0xFF).create(bytearray())]
        )
    ...
    def test_write(self):
        ...
        self.assertEqual(
            list(
                bytearray(
                    b"\x02"
                    + b"\x00\x00\x26"  # type of message - server_hello
                    + b"\x01\x01"  # length
                    + b"\x00" * 31  # proto version
                    + b"\x02"
                    + b"\x00"  # random
                    + b"\x00\x04"  # session id length
                    + b"\x00"  # cipher suite  # compression method
                )
            ),
    ...
    def test_write_with_next_protos(self):
        ...
        self.assertEqual(
            list(
                bytearray(
                    b"\x02"
                    + b"\x00\x00\x3c"  # type of message - server_hello
                    + b"\x01\x01"  # length
                    + b"\x00" * 31  # proto version
                    + b"\x02"
                    + b"\x00"  # random
                    + b"\x00\x04"  # session id length
                    + b"\x00"  # cipher suite
                    + b"\x00\x14"  # compression method
                    + b"\x33\x74"  # extensions length
                    + b"\x00\x10"  # ext type - NPN (13172)
                    + b"\x06"  # ext length - 16 bytes
                    +  # first entry length - 6 bytes
    ...
    def test_write_tls1_3(self):
        ...
        self.assertEqual(
            list(
                bytearray(
                    b"\x02"
                    + b"\x00\x00\x2c"  # type - server_hello
                    + b"\x03\x04"  # overall length
                    + b"\x02" * 32  # protocol version
                    + b"\x04"  # random
                    + b"\x03" * 4  # session_id length
                    + b"\x00\x04"  # session_id
                    + b"\x00"  # cipher suite
    ...
    def test___str__(self):
        ...
        self.assertEqual(
            "server_hello,length(40),version(3.0),random(...),"
    ...
    def test___str___with_extension(self):
        ...
        self.assertEqual(
            "server_hello,length(55),version(3.0),random(...),"
            "session ID(bytearray(b'\\x01 ')),cipher(0x86c4),"
...
class TestServerHello2(unittest.TestCase):
    ...
    def test_create(self):
        ...
        sh = sh.create(1, 2, (3, 4), bytearray(b"\x05"), [6, 7], bytearray(b"\x08\x09"))
    ...
    def test_write(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x04"
                + b"\x01"  # type - SERVER-HELLO
                + b"\x02"  # session ID hit
                + b"\x03\x04"  # certificate_type
                + b"\x00\x01"  # version
                + b"\x00\x06"  # certificate length
                + b"\x00\x02"  # ciphers length
                + b"\x05"  # session ID length
                + b"\x00\x00\x06"  # certificate
                + b"\x00\x00\x07"  # first cipher
                + b"\x08\x09"  # second cipher  # session ID
            ),
            sh.write(),
        )
    ...
    def test_write_with_invalid_version(self):
        sh = ServerHello2()
        sh = sh.create(
            1, 2, (3, 4, 12), bytearray(b"\x05"), [6, 7], bytearray(b"\x08\x09")
        )
...
class TestRecordHeader2(unittest.TestCase):
    ...
    def test_parse(self):
        parser = Parser(bytearray(b"\x80" + b"\x12"))  # head  # length
    ...
    def test_parse_with_invalid_header(self):
        parser = Parser(bytearray(b"\x00" + b"\x12"))  # header (bad)  # length
    ...
    def test_parse_with_very_long_message(self):
        parser = Parser(bytearray(b"\x82" + b"\x00"))  # header and a nibble of length
    ...
    def test_parse_with_3_byte_long_header(self):
        parser = Parser(
            bytearray(
                b"\x02"
                + b"\x00"  # 3 byte header and nibble of length
                + b"\x0a"  # second byte of length  # padding length
            )
        )
    ...
    def test_parse_with_2_byte_header_and_security_escape_bit_set(self):
        parser = Parser(bytearray(b"\xc0" + b"\x12"))
    ...
    def test_parse_with_3_byte_header_and_security_escape_bit_set(self):
        parser = Parser(bytearray(b"\x40" + b"\x12" + b"\x01"))
    ...
    def test_write(self):
        ...
        self.assertEqual(bytearray(b"\x81" b"\x23"), data)
    ...
    def test_write_with_padding(self):
        ...
        self.assertEqual(bytearray(b"\x01" b"\x23" b"\x0c"), data)
    ...
    def test_write_with_security_escape(self):
        ...
        self.assertEqual(bytearray(b"\x41" b"\x23" b"\x00"), data)
    ...
    def test_write_with_large_data_and_short_header(self):
        ...
        self.assertEqual(bytearray(b"\xff" b"\xff"), data)
    ...
    def test_write_with_long_length_and_long_header(self):
        ...
        self.assertEqual(bytearray(b"\x3f" b"\xff" b"\x01"), data)
...
class TestRecordHeader3(unittest.TestCase):
    ...
    def test_write(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x17"
                + b"\x03\x03"  # protocol type
                + b"\x00\x0a"  # protocol version  # length
            ),
            rh.write(),
        )
    ...
    def test_parse(self):
        parser = Parser(
            bytearray(
                b"\x17"
                + b"\x03\x03"  # protocol type - app data
                + b"\x00\x0f"  # protocol version  # length
            )
        )
    ...
    def test___str__(self):
        ...
        self.assertEqual(
            "SSLv3 record,version(3.0),content type(handshake)," + "length(12)", str(rh)
        )
    ...
    def test___str___with_invalid_content_type(self):
        ...
        self.assertEqual(
            "SSLv3 record,version(3.3)," + "content type(unknown(12)),length(0)",
            str(rh),
        )
    ...
    def test___repr__(self):
        ...
        self.assertEqual("RecordHeader3(type=23, version=(3.0), length=256)", repr(rh))
...
class TestAlert(unittest.TestCase):
    ...
    def test_levelName(self):
        alert = Alert().create(AlertDescription.record_overflow, AlertLevel.fatal)
    ...
    def test___str__(self):
        ...
        self.assertEqual("Alert, level:fatal, description:record_overflow", str(alert))
    ...
    def test_parse(self):
        ...
        parser = Parser(bytearray(b"\x01" + b"\x02"))  # level  # description
...
class TestClientKeyExchange(unittest.TestCase):
    def test___init__(self):
        ...
        self.assertEqual(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, cke.cipherSuite)
    ...
    def test_createSRP(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x10"
                + b"\x00\x00\x13"  # CKE
                + b"\x00\x11"  # Handshake message length
                + b"\x01"  # length of value
                + b"\x00" * 15  # 2...
                + b"\x03"  # ...**128...
            ),
        )  # ...+3
    ...
    def test_createRSA(self):
        cke = ClientKeyExchange(CipherSuite.TLS_RSA_WITH_AES_256_CBC_SHA, (3, 3))
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x10"
                + b"\x00\x00\x0e"  # CKE
                + b"\x00\x0c"  # Handshake message length
                + b"\x00" * 12  # length of encrypted value
            ),
        )

    def test_createRSA_with_SSL3(self):
        cke = ClientKeyExchange(CipherSuite.TLS_RSA_WITH_AES_256_CBC_SHA, (3, 0))
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x10"
                + b"\x00\x00\x0c"  # CKE
                + b"\x00" * 12  # Handshake message length
            ),
        )
    ...
    def test_write_with_DHE_RSA(self):
        cke = ClientKeyExchange(CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA, (3, 1))
    ...
    def test_parse_with_RSA(self):
        cke = ClientKeyExchange(CipherSuite.TLS_RSA_WITH_3DES_EDE_CBC_SHA, (3, 1))

        parser = Parser(bytearray(b"\x00\x00\x0e" + b"\x00\x0c" + b"\x00" * 12))
    ...
    def test_parse_with_RSA_in_SSL3(self):
        cke = ClientKeyExchange(CipherSuite.TLS_RSA_WITH_3DES_EDE_CBC_SHA, (3, 0))

        parser = Parser(bytearray(b"\x00\x00\x0c" + b"\x00" * 12))
    ...
    def test_parse_with_SRP(self):
        ...
        parser = Parser(
            bytearray(b"\x00\x00\x0a" + b"\x00\x08" + b"\x00" * 7 + b"\xff")
        )
    ...
    def test_parse_with_DH(self):
        ...
        parser = Parser(
            bytearray(b"\x00\x00\x0a" + b"\x00\x08" + b"\x01" + b"\x00" * 7)
        )
    ...
    def test_parse_with_ECDH(self):
        ...
        parser = Parser(bytearray(b"\x00\x00\x04" b"\x03" b"\x04\xff\xcd"))
    ...
    def test_parse_with_unknown_cipher(self):
        ...
        parser = Parser(bytearray(b"\x00\x00\x00"))
...
class TestClientMasterKey(unittest.TestCase):
    def test___init__(self):
        ...
        self.assertEqual(cmk.handshakeType, SSL2HandshakeType.client_master_key)

    def test_create(self):
        cmk = ClientMasterKey()
        cmk = cmk.create(
            1, bytearray(b"\x02"), bytearray(b"\x03\x04"), bytearray(b"\x05\x06\x07")
        )
    ...
    def test_write(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x02"
                + b"\x00\x00\x01"  # message type
                + b"\x00\x01"  # cipher spec
                + b"\x00\x02"  # clear key length
                + b"\x00\x03"  # encrypted key length
                + b"\x02"  # key argument length
                + b"\x03\x04"  # clear key
                + b"\x05\x06\x07"  # encrypted key  # key argument
            ),
            cmk.write(),
        )
...
class TestServerKeyExchange(unittest.TestCase):
    def test___init__(self):
        ske = ServerKeyExchange(CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA, (3, 1))

        self.assertIsNotNone(ske)
        self.assertEqual(ske.cipherSuite, CipherSuite.TLS_RSA_WITH_AES_128_CBC_SHA)
    ...
    def test___repr__(self):
        ske = ServerKeyExchange(CipherSuite.TLS_DH_ANON_WITH_AES_128_CBC_SHA, (3, 1))
    ...
    def test___repr___with_DHE(self):
        ske = ServerKeyExchange(CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA256, (3, 3))
    ...
    def test___repr___with_SRP(self):
        ske = ServerKeyExchange(CipherSuite.TLS_SRP_SHA_WITH_AES_128_CBC_SHA, (3, 3))

        ske.createSRP(srp_N=1, srp_g=2, srp_s=bytearray(3), srp_B=4)
    ...
    def test_createSRP(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x0c"
                + b"\x00\x00\x0d"  # message type
                + b"\x00\x01"  # overall length
                + b"\x01"  # N parameter length
                + b"\x00\x01"  # N value
                + b"\x02"  # g parameter length
                + b"\x03"  # g value
                + b"\x00" * 3  # s parameter length
                + b"\x00\x01"  # s value
                + b"\x04"  # B parameter length  # B value
            ),
        )

    def test_createSRP_with_signature(self):
        ske = ServerKeyExchange(
            CipherSuite.TLS_SRP_SHA_RSA_WITH_AES_128_CBC_SHA, (3, 1)
        )
        ...
        self.assertEqual(
            ...
            bytearray(
                ...
                + b"\x00\x01"  # s value
                + b"\x04"  # B parameter length  # B value
                b"\x00\x03" + b"\xc0\xff\xee"  # signature length  # signature value
            ),
        )

    def test_createSRP_with_signature_in_TLS_v1_2(self):
        ske = ServerKeyExchange(
            CipherSuite.TLS_SRP_SHA_RSA_WITH_AES_128_CBC_SHA, (3, 3)
        )
        ...
        self.assertEqual(
            ...
            bytearray(
                ...
                + b"\x00" * 3  # s parameter length
                + b"\x00\x01"  # s value
                + b"\x04"  # B parameter length
                + b"\x06\x01"  # B value
                + b"\x00\x03"  # SHA512+RSA
                + b"\xc0\xff\xee"  # signature length  # signature value
            ),
        )

    def test_createDH(self):
        ske = ServerKeyExchange(CipherSuite.TLS_DH_ANON_WITH_AES_128_CBC_SHA, (3, 3))
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x0c"
                + b"\x00\x00\x09"  # message type
                + b"\x00\x01"  # overall length
                + b"\x1f"  # p parameter length
                + b"\x00\x01"  # p value
                + b"\x02"  # g parameter length
                + b"\x00\x01"  # g value
                + b"\x10"  # Ys parameter length  # Ys value
            ),
        )

    def test_createECDH(self):
        ske = ServerKeyExchange(CipherSuite.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA, (3, 3))

        point = bytearray(b"\x04\x0a\x0a\x0b\x0b")
        ske.createECDH(
            ECCurveType.named_curve, named_curve=GroupName.secp256r1, point=point
        )
    ...
    def test_createDH_DSA(self):
        ske = ServerKeyExchange(CipherSuite.TLS_DHE_DSS_WITH_AES_256_GCM_SHA384, (3, 3))
    ...
    def test_parse_with_unknown_cipher(self):
        ...
        parser = Parser(bytearray(b"\x00\x00\x03" + b"\x00\x01" + b"\xff"))
    ...
    def test_parse_and_write_with_zero_padded_SRP(self):
        ...
        msg = bytearray(
            ...
            + b"\x00" * 3  # s parameter length
            + b"\x00\x02"  # s value
            + b"\x00\x04"  # B parameter length  # B value
        )
    ...
    def test_parser_read_write_with_zero_padded_DH(self):
        ...
        msg = bytearray(
            b"\x00\x00\x0c"
            + b"\x00\x02"  # overall length
            + b"\x00\x1f"  # p parameter length
            + b"\x00\x02"  # p value
            + b"\x00\x02"  # g parameter length
            + b"\x00\x02"  # g value
            + b"\x00\x10"  # Ys parameter length  # Ys value
        )
    ...
    def test_parser_with_DH_DSA(self):
        ske = ServerKeyExchange(CipherSuite.TLS_DHE_DSS_WITH_AES_128_CBC_SHA, (3, 2))
    ...
    def test_hash_with_ecdsa_in_tls1_1(self):
        ske = ServerKeyExchange(
            CipherSuite.TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA, (3, 2)
        )
        ...
        self.assertEqual(
            hash1, bytearray(b" \xa0\xc1P5\xf7K/\xednd" b"\xbaQ\xedo\xa13Z\xa5}")
        )

    def test_hash_with_dsa_in_tls1_2(self):
        ske = ServerKeyExchange(CipherSuite.TLS_DHE_DSS_WITH_AES_128_CBC_SHA256, (3, 3))
        ...
        self.assertEqual(
            hash1,
            bytearray(
                b"\x4a\x95\x6f\x85\x4e\xed\x6c\x5f\x84\x17\x92\xd6\xa8\x8d\x8c\xd7\xb2\xd0\xf7\xfb"
            ),
        )

    def test_hash_with_rsa_pss_sha256(self):
        ske = ServerKeyExchange(CipherSuite.TLS_DHE_RSA_WITH_AES_128_CBC_SHA, (3, 3))
        ...
        self.assertEqual(
            hash1,
            bytearray(
                b"^\xfe\x0e\x8f\xd2\x87\x8e/%\xbeK9\xb7$"
    ...
    def test_hash_with_sha1_hash_algorithm_for_TLS_v1_2(self):
        ...
        self.assertEqual(
            hash1, bytearray(b"\x8e?YW\xcd\xad\xc6\x83\x91\x1d.fe,\x17y=\xc4T\x89")
        )
...
class TestCertificateRequest(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        parser = Parser(
            bytearray(
                ...
                + b"\x00\x00"  # type rsa_sign  # length of CA list
            )
        )
    ...
    def test_parse_with_TLSv1_2(self):
        ...
        parser = Parser(
            bytearray(
                ...
                + b"\x01"  # overall length
                + b"\x01"  # length of certificate types
                + b"\x00\x0a"  # type rsa_sign
                + b"\x06\x01"  # length of signature types
                + b"\x05\x01"  # SHA512+RSA
                + b"\x04\x01"  # SHA384+RSA
                + b"\x03\x01"  # SHA256+RSA
                + b"\x02\x01"  # SHA224+RSA
                + b"\x00\x0a"  # SHA1+RSA
                + b"\x00" * 10  # length of CA list  # opaque data type
            )
        )
    ...
    def test_parse_with_TLS1_3(self):
        ...
        parser = Parser(
            bytearray(
                ...
                + b"\x00\x0a"  # context data
                + b"\x00\x0d"  # length of extensions
                + b"\x00\x06"  # extension type signature_algorithms(13)
                + b"\x00\x04"  # extension data length
                + b"\x08\x06"  # sig alg list length in bytes
                + b"\x02\x01"  # rsa_pss_rsae_sha512  # rsa_pkcs1_sha1
            )
        )
        ...
        self.assertEqual(cr.certificate_request_context, b"\x01")
        self.assertEqual(
            cr.extensions, [SignatureAlgorithmsExtension().create([(8, 6), (2, 1)])]
        )
    ...
    def test_parse_with_TLS1_3_invalid(self):
        ...
        parser = Parser(
            bytearray(
                b"\x00\x00\x02"
                + b"\x01"  # overall length
                + b"\x01"  # length of certificate_request_context  # context data
            )
        )
    ...
    def test_write(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                ...
                + b"\x01"  # overall length
                + b"\x01"  # length of certificate types
                + b"\x00\x04"  # type rsa sign
                + b"\x00\x02"  # length of CA list
                + b"\xff\xff"  # length of entry  # opaque
            ),
        )
    ...
    def test_write_in_TLS_v1_2(self):
        ...
        cr.create([ClientCertificateType.rsa_sign], [], [(6, 1), (4, 1), (2, 1)])
        ...
        self.assertEqual(
            ...
            bytearray(
                ...
                + b"\x01"  # overall length
                + b"\x01"  # length of certificate types
                + b"\x00\x06"  # type rsa sign
                + b"\x06\x01"  # signature types
                + b"\x04\x01"  # SHA512+RSA
                + b"\x02\x01"  # SHA256+RSA
                + b"\x00\x00"  # SHA1+RSA  # length of CA list
            ),
        )
    ...
    def test_write_in_TLS_v1_3(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x0d"
                + b"\x00\x00\x0d"  # type
                + b"\x00"  # overall length
                + b"\x00\x0a"  # length of self.certificate_request_context
                + b"\x00\x0d"  # length of extensions
                + b"\x00\x06"  # extension type signature_algorithms(13)
                + b"\x00\x04"  # extension data length
                + b"\x02\x01"  # sig alg list length in bytes
                + b"\x08\x06"  # rsa_pkcs1_sha1  # rsa_pss_rsae_sha512
            ),
        )
...
class TestCertificateVerify(unittest.TestCase):
    ...
    def test_write(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                ...
                + b"\x00\x00\x04"  # type
                + b"\x00\x02"  # overall length
                + b"\xf0\x0f"  # length of signature  # signature
            ),
        )
    ...
    def test_parse_with_TLSv1_2(self):
        ...
        parser = Parser(
            bytearray(
                ...
                + b"\x02\x01"  # length
                + b"\x00\x02"  # SHA1 + RSA
                + b"\xab\xcd"  # length of signature  # signature
            )
        )
        ...
        self.assertEqual(
            cv.signatureAlgorithm, (HashAlgorithm.sha1, SignatureAlgorithm.rsa)
        )
    ...
    def test_write_with_TLSv1_2(self):
        ...
        cv.create(
            bytearray(b"\xff\xba"), (HashAlgorithm.sha512, SignatureAlgorithm.rsa)
        )
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x0f"
                + b"\x00\x00\x06"  # type
                + b"\x06\x01"  # overall length
                + b"\x00\x02"  # SHA512+RSA
                + b"\xff\xba"  # signature length  # signature
            ),
        )
...
class TestServerHelloDone(unittest.TestCase):
    ...
    def test_write(self):
        ...
        self.assertEqual(bytearray(b"\x0e\x00\x00\x00"), shd.write())
...
class TestFinished(unittest.TestCase):
    ...
    def test_parse_ssl2(self):
        ...
        parser = Parser(bytearray(b"\x00\x00\x24" + b"\x0f" * 0x24))
    ...
    def test_write_ssl3(self):
        ...
        self.assertEqual(
            finished.write(), bytearray(b"\x14" + b"\x00\x00\x24" + b"\x03" * 36)
        )
    ...
    def test_write_tls_1_2(self):
        ...
        self.assertEqual(
            finished.write(), bytearray(b"\x14" + b"\x00\x00\x0c" + b"\x04" * 12)
        )
    ...
    def test_write_tls_1_3(self):
        ...
        self.assertEqual(
            finished.write(), bytearray(b"\x14" + b"\x00\x00\x20" + b"\x04" * 32)
        )
...
class TestClientFinished(unittest.TestCase):
    ...
    def test_write(self):
        ...
        self.assertEqual(
            bytearray(b"\x03" + b"\xc0\xfe"),  # message type  # message payload
            fin.write(),
        )
...
class TestServerFinished(unittest.TestCase):
    ...
    def test_write(self):
        ...
        self.assertEqual(
            bytearray(b"\x06" + b"\xc0\xfe"),  # message type  # message payload
            fin.write(),
        )
...
class TestCertificateStatus(unittest.TestCase):
    ...
    def test___init__(self):
        ...
        self.assertEqual(self.msg.handshakeType, 22)
    ...
    def test_write(self):
        ...
        self.assertEqual(
            self.msg.write(),
            bytearray(b"\x16" b"\x00\x00\x06" b"\x01" b"\x00\x00\x02" b"\xbc\xaa"),
        )
...
class TestCertificateEntry(unittest.TestCase):
    ...
    def test_write(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x00\x01\xfa"
                + self.der_cert  # length of certificate
                + b"\x00\x06"
                + b"\x00\xff"  # length of extensions
                + b"\x00\x02"  # type of first extension
                + b"\xde\xad"  # length of first extension
            ),
        )  # extension payload
    ...
    def test_write_without_extensions(self):
        ...
        self.assertEqual(
            entry.write(),
            bytearray(
                b"\x00\x01\xfa" + self.der_cert + b"\x00\x00"  # length of certificate
            ),
    ...
    def test_write_with_null_extensions(self):
        ...
        self.assertEqual(
            entry.write(),
            bytearray(b"\x00\x01\xfa" + self.der_cert),  # length of certificate
        )
...
class TestCertificate(unittest.TestCase):
    ...
    def test___repr__(self):
        cert = Certificate(CertificateType.x509)
        cert = cert.create(X509CertChain([bytearray(b"one"), bytearray(b"two")]))
        self.assertEqual(
            repr(cert),
            "Certificate(cert_chain=[bytearray(b'one'), " "bytearray(b'two')])",
        )
    ...
    def test_write_empty(self):
        ...
        self.assertEqual(
            cert.write(),
            bytearray(
                b"\x0b"  # type of message - certificate
                b"\x00\x00\x03"  # length of message - 3 bytes
                b"\x00\x00\x00"
            ),
        )  # length of the list of certs
    ...
    def test_write_none(self):
        ...
        self.assertEqual(
            cert.write(),
            bytearray(
                b"\x0b" b"\x00\x00\x03" b"\x00\x00\x00"  # length of message - 3 bytes
            ),
        )  # length of the list of certs
    ...
    def test_parse_empty(self):
        ...
        parser = Parser(
            bytearray(  # b'\x0b'  # type of message - certificate
                b"\x00\x00\x03" b"\x00\x00\x00"  # length of message - 3 bytes
            )
        )  # length of the list of certs
    ...
    def test_write_with_cert(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x0b"
                + b"\x00\x02\x00"  # type of message - certificate
                + b"\x00\x01\xfd"  # length of handshake message
                + b"\x00\x01\xfa"  # length of array of certificates
                + self.der_cert  # length of the first certificate
            ),
        )
    ...
    def test_write_tls13_with_cert(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x0b"
                + b"\x00\x02\x09"  # type of message - certificate
                + b"\x00"  # length of handshake message
                + b"\x00\x02\x05"  # length of certificate request context
                + b"\x00\x01\xfa"  # length of certificate list
                + self.der_cert  # length of certificate
                + b"\x00\x06"
                + b"\x00\xff"  # length of extensions
                + b"\x00\x02"  # type of first extension
    ...
    def test_parse_with_cert(self):
        ...
        self.assertEqual(cert.cert_chain.x509List[0].writeBytes(), self.der_cert)
...
class TestNextProtocol(unittest.TestCase):
    ...
    def test_write(self):
        ...
        self.assertEqual(
            bytearray(
                b"\x43"
                + b"\x00\x00\x20"  # type - NPN
                + b"\x04"  # length - 32 bytes
                + b"test"  # value length
                + b"\x1a"  # value
                + b"\x00" * 0x1A  # length of padding  # padding
            ),
            np.write(),
        )
...
class TestEncryptedExtensions(unittest.TestCase):
    ...
    def test_parse_with_extension(self):
        ...
        self.assertEqual(
            ext.extensions[0].groups, [GroupName.secp256r1, GroupName.x25519]
        )
    ...
    def test_write(self):
        ext = SupportedGroupsExtension().create([GroupName.secp256r1, GroupName.x25519])
        self.msg.create([ext])

        self.assertEqual(
            self.msg.write(),
            bytearray(
                b"\x08"  # handshake type - encrypted extensions
                b"\x00\x00\x0c"  # overall length
                b"\x00\x0a"  # extensions list length
                b"\x00\x0a"  # supported groups extension
                b"\x00\x06"  # key share extension length
...
class TestSessionTicketPayload(unittest.TestCase):
    ...
    def test_write(self):
        ticket = SessionTicketPayload()
        ticket.create(
            bytearray(b"\x22" * 32), (3, 4), 0x1302, 12345678, bytearray(b"\x11" * 16)
        )
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x00\x00"
                + b"\x00\x20"  # version (internal)
                + b"\x22" * 32  # master secret length
                + b"\x03\x04"  # master secret
                + b"\x13\x02"  # protocol version
                + b"\x10"  # cipher suite
                + b"\x11" * 16  # nonce length
                + b"\x00\x00\x00\x00\x00\xbc\x61\x4e"  # nonce
            ),
        )  # creation time
    ...
    def test_write_v1(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                ...
                + b"\x11" * 16  # nonce length
                + b"\x00\x00\x00\x00\x00\xbc\x61\x4e"  # nonce
                + b"\x00\x01\xff"  # creation time
                + b"\x00\x01\xfa"  # length of array of certificates
                + self.der_cert  # length of certificate
                + b"\x00\x00"  # certificate payload
            ),
        )
    ...
    def test_write_v2(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"\x00\x02"
                + b"\x00\x20"  # version
                + b"\x11" * 32  # master secret length
                + b"\x03\x03"  # master secret
                + b"\x00\xaf"  # protocol version
                + b"\x00"  # cipher suite
                + b"\x00\x00\x00\x00\x00\xbc\x61\x4e"  # nonce length
                + b"\x00\x00\x00"  # creation time
                + b"\x01"  # length of certificate
                + b"\x00"  # encrypt then mac
                + b"\x00\x00"  # extended master secret
            ),
        )  # server name length
    ...
    def test_parse_v1(self):
        ...
        self.assertEqual(
            ticket.client_cert_chain.x509List[0].writeBytes(), self.der_cert
        )
...
class TestNewSessionTicket(unittest.TestCase):
    ...
    def test_write(self):
        ...
        ticket = ticket.create(
            ...
            bytearray(b"abc"),
            bytearray(b"ticket"),
            [TLSExtension(extType=ExtensionType.early_data).create(bytearray())],
        )
...
class TestHeartbeat(unittest.TestCase):
    ...
    def test_messageType(self):
        heartbeat_msg = Heartbeat().create(
            HeartbeatMessageType.heartbeat_request, bytearray(1), 20
        )
    ...
    def test___str__(self):
        heartbeat_msg = Heartbeat().create(
            HeartbeatMessageType.heartbeat_response, bytearray(1), 20
        )
    ...
    def test_parse_with_missing_data(self):
        ...
        parser = Parser(bytearray(b"\x01" + b"\x00\x01"))
    ...
    def test_write(self):
        heartbeat_msg = Heartbeat().create(
            HeartbeatMessageType.heartbeat_request, b"\xAA", 20
        )

        self.assertEqual(
            heartbeat_msg.write()[:4],
            b"\x01" + b"\x00\x01" + b"\xAA",  # request  # payload len
        )  # payload
    ...
    def test_create_response(self):
        ...
        self.assertEqual(
            heartbeat_response.message_type, HeartbeatMessageType.heartbeat_response
        )
...
class TestHelloRequest(unittest.TestCase):
    ...
    def test_write(self):
        self.assertEqual(
            self.msg.write(),
            bytearray(b"\x00" b"\x00\x00\x00"),  # handshake type  # overall length
        )

    def test_parse(self):
        parser = Parser(
            bytearray(b"\x00\x00\x00")  #  b'\x00'  # type
        )  # overall length
    ...
    def test_parse_with_truncated_length(self):
        parser = Parser(
            bytearray(b"\x00\x00")  #  b'\x00'  # type
        )  # overall length (truncated)
        with self.assertRaises(SyntaxError):
            self.msg.parse(parser)

    def test_parse_with_non_zero_payload(self):
        parser = Parser(
            bytearray(b"\x00\x00\x01" b"\xff")  #  b'\x00'  # type  # overall length
        )  # some garbage
        with self.assertRaises(SyntaxError):
            self.msg.parse(parser)
...
class TestKeyUpdate(unittest.TestCase):
    ...
    def test_parse(self):
        parser = Parser(
            bytearray(  # b'\x18'  # type
                b"\x00\x00\x01" b"\x00"  # overall length  # message_type
            )
        )
    ...
    def test_parse_with_truncated_payload(self):
        parser = Parser(
            bytearray(b"\x00\x00\x01")  # b'\x18'  # type  # overall length (invalid)
        )
    ...
    def test_parse_with_too_long_payload(self):
        parser = Parser(
            bytearray(b"\x00\x00\x02" b"\x00\x00")  # b'\x18'  # type  # overall length
        )
