...
class TestNumberToBytesFunctions(unittest.TestCase):
    def test_numberToByteArray(self):
        self.assertEqual(numberToByteArray(0x00000000000001), bytearray(b"\x01"))

    def test_numberToByteArray_with_MSB_number(self):
        self.assertEqual(numberToByteArray(0xFF), bytearray(b"\xff"))

    def test_numberToByteArray_with_length(self):
        self.assertEqual(numberToByteArray(0xFF, 2), bytearray(b"\x00\xff"))

    def test_numberToByteArray_with_not_enough_length(self):
        self.assertEqual(numberToByteArray(0x0A0B0C, 2), bytearray(b"\x0b\x0c"))
    ...
    def test_small_number(self, number):
        self.assertEqual(
            numberToByteArray(number, 1), bytearray(struct.pack(">B", number))
        )
    ...
    def test_very_large_number(self):
        self.assertEqual(numberToByteArray((1 << 128) - 1), bytearray(b"\xff" * 16))
    ...
    def test_small_number_little_endian(self, number):
        self.assertEqual(
            numberToByteArray(number, 1, endian="little"),
    ...
    def test_big_number(self, number):
        self.assertEqual(
            numberToByteArray(number, 4, endian="little"),
            bytearray(struct.pack("<L", number)),
        )

    def test_very_large_number(self):
        self.assertEqual(
            numberToByteArray((1 << 128) - 1, endian="little"), bytearray(b"\xff" * 16)
        )

    def test_numberToByteArray_with_not_enough_length_little_endian(self):
        self.assertEqual(
            numberToByteArray(0x0A0B0C, 2, endian="little"), bytearray(b"\x0c\x0b")
        )

    def test_with_large_number_of_bytes_in_little_endian(self):
        self.assertEqual(
            numberToByteArray(1, 16, endian="little"), bytearray(b"\x01" + b"\x00" * 15)
        )
...
class TestHMACMethods(unittest.TestCase):
    def test_HMAC_MD5(self):
        self.assertEqual(
            HMAC_MD5(b"abc", b"def"),
            bytearray(b"\xde\xbd\xa7{|\xc3\xe7\xa1\x0e\xe7" b"\x01\x04\xe6qzk"),
        )

    def test_HMAC_SHA1(self):
        self.assertEqual(
            HMAC_SHA1(b"abc", b"def"),
            bytearray(
                b"\x12UN\xab\xba\xf7\xe8\xe1.G7\x02" b"\x0f\x98|\xa7\x90\x10\x16\xe5"
            ),
        )

    def test_HMAC_SHA256(self):
        self.assertEqual(
            HMAC_SHA256(b"abc", b"def"),
            bytearray(
                b" \xeb\xc0\xf0\x93DG\x014\xf3P@\xf6>"
    ...
    def test_HMAC_SHA384(self):
        self.assertEqual(
            HMAC_SHA384(b"abc", b"def"),
            bytearray(
                b"\xec\x14\xd6\x94\x86\tHp\x84\x07\xect\x0e"
    ...
    def test_HMAC_expand_1(self):
        ...
        self.assertEqual(
            HKDF_expand(
                ...
                numberToByteArray(0xF0F1F2F3F4F5F6F7F8F9, 10),
                42,
                "sha256",
            ),
            numberToByteArray(
    ...
    def test_HMAC_expand_4(self):
        ...
        self.assertEqual(
            HKDF_expand(
                numberToByteArray(
                    int("0x9b6c18c432a7bf8f" "0e71c8eb88f4b30baa" "2ba243", 16), 20
                ),
                numberToByteArray(int("0xf0f1f2f3f4f5f6f7" "f8f9", 16), 10),
                42,
                "sha1",
            ),
            numberToByteArray(
    ...
    def test_HMAC_expand_5(self):
        ...
        self.assertEqual(
            HKDF_expand(
                numberToByteArray(
                    int("0x8adae09a2a307059" "478d309b26c4115a22" "4cfaf6", 16), 20
                ),
                numberToByteArray(
                    int(
                        "0xb0b1b2b3b4b5b6b7"
    ...
    def test_HMAC_expand_6(self):
        ...
        self.assertEqual(
            HKDF_expand(
                numberToByteArray(
                    int("0xda8c8a73c7fa7728" "8ec6f5e7c297786aa0" "d32d01", 16), 20
                ),
                bytearray(),
                42,
                "sha1",
    ...
    def test_HMAC_expand_7(self):
        ...
        self.assertEqual(
            HKDF_expand(
                numberToByteArray(
                    int("0x2adccada18779e7c" "2077ad2eb19d3f3e73" "1385dd", 16), 20
                ),
                bytearray(),
                42,
                "sha1",
...
class TestHashMethods(unittest.TestCase):
    def test_MD5(self):
        self.assertEqual(
            ...
            bytearray(
                b"\xf9\x6b\x69\x7d\x7c\xb7\x93\x8d" b"\x52\x5a\x2f\x31\xaa\xf1\x61\xd0"
            ),
        )
...
class TestBytesToNumber(unittest.TestCase):
    ...
    def test_small_numbers(self, number):
        self.assertEqual(bytesToNumber(bytearray(struct.pack(">B", number))), number)

    @given(integers(min_value=0, max_value=0xFFFFFFFF))
    @example(0xFFFFFFFF)
    def test_multi_byte_numbers(self, number):
        self.assertEqual(bytesToNumber(bytearray(struct.pack(">I", number))), number)

    def test_very_long_numbers(self):
        self.assertEqual(bytesToNumber(bytearray(b"\x00" * 16 + b"\x80")), 0x80)
        self.assertEqual(
            bytesToNumber(bytearray(b"\x80" + b"\x00" * 16)), 1 << (8 * 16 + 7)
        )
        self.assertEqual(bytesToNumber(bytearray(b"\xff" * 16)), (1 << (8 * 16)) - 1)
    ...
    def test_small_numbers_little_endian(self, number):
        self.assertEqual(
            bytesToNumber(bytearray(struct.pack("<B", number)), "little"), number
        )
    ...
    def test_multi_byte_numbers_little_endian(self, number):
        self.assertEqual(
            bytesToNumber(bytearray(struct.pack("<I", number)), "little"), number
        )

    def test_very_long_numbers_little_endian(self):
        self.assertEqual(
            bytesToNumber(bytearray(b"\x80" + b"\x00" * 16), "little"), 0x80
        )
        self.assertEqual(
            bytesToNumber(bytearray(b"\x00" * 16 + b"\x80"), "little"),
            1 << (8 * 16 + 7),
        )
        self.assertEqual(
            bytesToNumber(bytearray(b"\xff" * 16), "little"), (1 << (8 * 16)) - 1
        )
...
class TestHKDF_expand_label(unittest.TestCase):
    def test_with_sha256(self):
        ...
        self.assertEqual(
            HKDF_expand_label(secret, label, hash_value, length, "sha256"),
            bytearray(
                b"r\x91M\x13~\xd1\xa7\xf0\xa3\xa3\x0f\xce#"
    ...
    def test_with_sha384(self):
        ...
        self.assertEqual(
            HKDF_expand_label(secret, label, hash_value, length, "sha384"),
            bytearray(
                b"\xb3\rFt\x10\xd96b\xb3\x80pm3g\xc06\xc3"
...
class TestDerive_secret(unittest.TestCase):
    def test_with_no_hashes(self):
        ...
        self.assertEqual(
            ...
            bytearray(
                b"(\xef{\xee\xad\xde\x0b)9\xec\xb6\x89\xf5"
    ...
    def test_with_handshake_hashes(self):
        ...
        self.assertEqual(
            derive_secret(secret, label, handshake_hashes, algorithm),
            bytearray(
                b"\t\xec\x01W[Y\xdcP\xac\xebu\x13\xe6\x98"
