...
from tlslite.extensions import SNIExtension, PreSharedKeyExtension, PskIdentity
...
class TestHandshakeHelpers(unittest.TestCase):
    ...
    def test_alignClientHelloPadding_extension_list_initialization(self):
        ...
        self.assertEqual(
            bytearray(b"\x00\x15"), data[clientHelloLength + 2 : clientHelloLength + 4]
        )
    ...
    def test_update_binders_default_prf(self):
        ...
        self.assertIsInstance(clientHello.extensions[-1], PreSharedKeyExtension)
        ...
        self.assertEqual(
            ...
            [
                bytearray(
                    b"wOl\xbe\x9b\xca\xa4\xf3tS\x08M\ta\xa2t"
    ...
    def test_update_binders_sha384_prf(self):
        ...
        self.assertEqual(
            ...
            [
                bytearray(
                    b"\x8d\x92\xd2\xb7+D&\xd7\x0e>x\x1a\xc5i+"
    ...
    def test_update_binders_with_ticket(self):
        clientHello = ClientHello()
        clientHello.create((3, 3), bytearray(32), bytearray(0), [0])
        identities = [PskIdentity().create(bytearray(b"\x00ticket\x00ident"), 123)]
        binders = [bytearray(48)]
        psk_ext = PreSharedKeyExtension().create(identities, binders)
        clientHello.extensions = [psk_ext]

        ticket = NewSessionTicket().create(
            3600,  # ticket lifetime
            123,  # age_add
        ...
        HandshakeHelpers.update_binders(
            clientHello, hh, [], [ticket], resum_master_secret
        )
        ...
        self.assertEqual(
            ...
            [
                bytearray(
                    b"<\x03\xcd\xd5\xce\xaeo\x8d\xc6\x8c\xe3"
    ...
    def test_verify_binder_with_wrong_extension(self):
        ...
        with self.assertRaises(TLSIllegalParameterException) as e:
            HandshakeHelpers.verify_binder(clientHello, hh, 0, secret, "sha256")
    ...
    def test_verify_binder(self):
        ...
        ret = HandshakeHelpers.verify_binder(clientHello, hh, 0, secret, "sha384")
    ...
    def test_verify_binder_with_wrong_binder(self):
        ...
        with self.assertRaises(TLSIllegalParameterException) as e:
            HandshakeHelpers.verify_binder(clientHello, hh, 0, secret, "sha384")
