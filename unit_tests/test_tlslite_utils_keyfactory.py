...
class TestParsePEMKey(unittest.TestCase):
    ...
    def test_with_missing_m2crypto(self):
        with self.assertRaises(ValueError):
            key = parsePEMKey(
                self.privKey_str, private=True, implementations=["openssl"]
            )
    ...
    def test_key_parse_using_openssl(self):
        ...
        if not hasattr(M2Crypto, "version_info") or M2Crypto.version_info >= (0, 28, 0):
            exp = EVPError
        else:
            exp = SyntaxError
    ...
    def test_key_parse_with_new_lines_using_openssl(self):
        key = parsePEMKey(
            self.privKey_str_newLines, private=True, implementations=["openssl"]
        )
    ...
    def test_rsa_key_parse_using_openssl(self):
        ...
        with self.assertRaises(SyntaxError):
            key = parsePEMKey(
                self.privRSAKey_str, private=True, implementations=["openssl"]
            )
    ...
    def test_rsa_key_parse_with_new_lines_using_openssl(self):
        key = parsePEMKey(
            self.privRSAKey_str_newLines, private=True, implementations=["openssl"]
        )
    ...
    def test_key_parse_using_python(self):
        key = parsePEMKey(self.privKey_str, private=True, implementations=["python"])
    ...
    def test_key_parse_with_new_lines_using_python(self):
        key = parsePEMKey(
            self.privKey_str_newLines, private=True, implementations=["python"]
        )
    ...
    def test_rsa_key_parse_using_python(self):
        key = parsePEMKey(self.privRSAKey_str, private=True, implementations=["python"])
    ...
    def test_rsa_key_parse_with_new_lines_using_python(self):
        key = parsePEMKey(
            self.privRSAKey_str_newLines, private=True, implementations=["python"]
        )
    ...
    def test_rsa_pss_key_parse_using_openssl(self):
        try:
            key = parsePEMKey(
                self.privRSAPSSKey_str, private=True, implementations=["openssl"]
            )
        except SyntaxError:
    ...
    def test_rsa_pss_key_parse_using_python(self):
        key = parsePEMKey(
            self.privRSAPSSKey_str, private=True, implementations=["python"]
        )
