...
class TestPoly1305(unittest.TestCase):
    ...
    def test_le_bytes_to_num_32(self):
        self.assertEqual(
            0x01020304, Poly1305.le_bytes_to_num(bytearray(b"\x04\x03\x02\x01"))
        )

    def test_le_bytes_to_num_40(self):
        self.assertEqual(
            0x0001020304, Poly1305.le_bytes_to_num(bytearray(b"\x04\x03\x02\x01\x00"))
        )

    def test_le_bytes_to_num_64(self):
        self.assertEqual(
            0x0102030405060708,
            Poly1305.le_bytes_to_num(bytearray(b"\x08\x07\x06\x05\x04\x03\x02\x01")),
        )

    def test_le_bytes_to_num_72(self):
        self.assertEqual(
            0x0A0102030405060708,
    ...
    def test_vector7(self):
        ...
        message = bytearray(
            b"\xff" * 16 + b"\xf0" + b"\xff" * 15 + b"\x11" + b"\x00" * 15
        )
    ...
    def test_vector10(self):
        ...
        self.assertEqual(tag, bytearray(b"\x14" + b"\x00" * 7 + b"\x55" + b"\x00" * 7))
