...
class TestContanttimeCBCCheck(unittest.TestCase):
    @staticmethod
    def data_prepare(application_data, seqnum_bytes, content_type, version, mac, key):
        r_layer = RecordLayer(None)
        r_layer.version = version

        h = hmac.new(key, digestmod=mac)

        digest = r_layer.calculateMAC(h, seqnum_bytes, content_type, application_data)
    ...
    def test_with_empty_data_and_minimum_pad(self):
        ...
        data = self.data_prepare(
            application_data, seqnum_bytes, content_type, version, mac, key
        )
        ...
        self.assertTrue(
            ct_check_cbc_mac_and_pad(data, h, seqnum_bytes, content_type, version)
        )
...
class TestCompareDigest(unittest.TestCase):
    def test_with_equal_length(self):
        ...
        self.assertTrue(
            ct_compare_digest(bytearray(b"\x02" * 8), bytearray(b"\x02" * 8))
        )
    ...
    def test_different(self):
        self.assertFalse(ct_compare_digest(bytearray(b"\x01"), bytearray(b"\x03")))

        self.assertFalse(
            ct_compare_digest(
                bytearray(b"\x01" * 10 + b"\x02"), bytearray(b"\x01" * 10 + b"\x03")
            )
        )

        self.assertFalse(
            ct_compare_digest(
                bytearray(b"\x02" + b"\x01" * 10), bytearray(b"\x03" + b"\x01" * 10)
            )
        )
