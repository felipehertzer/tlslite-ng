    ...
    def _guard_against_unicode(self, data):
        ...
        if PY_VER < (3,):
            if isinstance(data, unicode):
                raise ValueError(
                    "Only bytes, bytearray or memoryview "
...
class Des(_baseDes):
    ...
    def __create_sub_keys(self):
        ...
        """
        key = self.__permutate(Des.__pc1, self.__string_to_bitlist(self.key))
...
class Python_TripleDES(_baseDes):
    ...
    def __init__(self, key, iv=None):
        ...
        if iv:
            if len(iv) != self.block_size:
                raise ValueError(
                    "Invalid Initialization Vector (iv) must be"
    ...
    def encrypt(self, data):
        ...
        if len(data) % self.block_size:
            raise ValueError(
                "Invalid data length, must be a multiple "
