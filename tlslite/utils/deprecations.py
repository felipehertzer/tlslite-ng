...
def deprecated_class_name(
    old_name, warn="Class name '{old_name}' is deprecated, " "please use '{new_name}'"
):
    """
    ...
    def _wrap(obj):
        ...
        def _warn():
            warnings.warn(
                warn.format(old_name=old_name, new_name=obj.__name__),
                DeprecationWarning,
                stacklevel=3,
            )
...
def deprecated_params(
    names, warn="Param name '{old_name}' is deprecated, " "please use '{new_name}'"
):
        ...
        def wrapper(*args, **kwargs):
            for new_name, old_name in names.items():
                if old_name in kwargs:
                    ...
                    warnings.warn(
                        warn.format(old_name=old_name, new_name=new_name),
                        DeprecationWarning,
                        stacklevel=2,
                    )
...
def deprecated_instance_attrs(
    names, warn="Attribute '{old_name}' is deprecated, " "please use '{new_name}'"
):
    ...
    def decorator(clazz):
        def getx(self, name, __old_getx=getattr(clazz, "__getattr__", None)):
            if name in names:
                warnings.warn(
                    warn.format(old_name=name, new_name=names[name]),
                    DeprecationWarning,
                    stacklevel=2,
                )
            ...
            raise AttributeError(
                "'{0}' object has no attribute '{1}'".format(clazz.__name__, name)
            )
...
def deprecated_method(message):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            warnings.warn(
                "{0} is a deprecated method. {1}".format(func.__name__, message),
                DeprecationWarning,
                stacklevel=2,
            )
            return func(*args, **kwargs)
