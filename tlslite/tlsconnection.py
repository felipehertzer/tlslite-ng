...
class TLSConnection(TLSRecordLayer):
    ...
    def handshakeClientAnonymous(
        self, session=None, settings=None, checker=None, serverName=None, async_=False
    ):
    ...
    def _handshakeClientAsyncHelper(
        ...
        if clientCertChain:
            ...
            if "x509" not in settings.certificateTypes:
                raise ValueError(
                    "Client certificate doesn't match " "Handshake Settings"
                )
        ...
        for result in self._clientResume(
            session, serverHello, clientHello.random, nextProto, settings
        ):
            if result in (0, 1):
                yield result
            else:
        ...
        # finishes reading the post-ServerHello messages.
            ...
            keyExchange = ECDHE_RSAKeyExchange(
                cipherSuite, clientHello, serverHello, None, acceptedCurves
            )
    ...
    def _clientSendClientHello(
        ...
        if settings.useEncryptThenMAC:
            extensions.append(
                TLSExtension().create(ExtensionType.encrypt_then_mac, bytearray(0))
            )
        if settings.useExtendedMasterSecret:
            extensions.append(
                TLSExtension().create(
                    ExtensionType.extended_master_secret, bytearray(0)
                )
            )
        ...
        if next((i for i in settings.versions if i > (3, 3)), None):
            ...
            if certParams and certParams[1]:
                extensions.append(
                    TLSExtension(extType=ExtensionType.post_handshake_auth).create(
                        bytearray(b"")
                    )
                )
        ...
        if (
            next(
                (
                    ...
                    if cipher in CipherSuite.ecdhAllSuites
                ),
                None,
            )
            is not None
        ...
        if settings.record_size_limit:
            extensions.append(
                RecordSizeLimitExtension().create(settings.record_size_limit)
            )
        ...
        if session and session.tls_1_0_tickets:
            ...
            for cached_ticket in session.tls_1_0_tickets:
                extensions.append(SessionTicketExtension().create(cached_ticket.ticket))
                break
            else:
                # or just advertise that we support session resumption
                extensions.append(SessionTicketExtension().create(bytearray(0)))
        else:
            extensions.append(SessionTicketExtension().create(bytearray(0)))
        ...
        if session and session.sessionID:
            ...
            if session.cipherSuite not in cipherSuites:
                raise ValueError(
                    "Session's cipher suite not consistent " "with parameters"
                )
            else:
        ...
        if (
            settings.pskConfigs or (session and session.tickets)
            ...
            if session and session.tickets:
                ...
                session.tickets[:] = (
                    i
                    for i in session.tickets
                    if i.time + i.ticket_lifetime > now
                    and i.time + 7 * 24 * 60 * 60 > now
                )
                if session.tickets:
                    ...
                    binder_len = (
                        48 if session.cipherSuite in CipherSuite.sha384PrfSuites else 32
                    )
    ...
    def _clientGetServerHello(self, settings, session, clientHello):
        ...
        if result.random == TLS_1_3_HRR and ext and ext.version > (3, 3):
            ...
            bad_ext = next(
                (i for i in hello_retry.extensions if i.extType not in ch_ext_types),
                None,
            )
            if bad_ext:
                ...
                for result in self._sendError(
                    ...
                    ("Unexpected extension in HRR: " "{0}").format(bad_ext),
                ):
                    yield result
            ...
            if sr_key_share_ext:
                ...
                if group_id not in groups_ext.groups:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Server selected group we " "did not advertise",
                    ):
                        yield result
                ...
                if next(
                    (
                        entry
                        for entry in cl_key_share_ext.client_shares
                    ...
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Server selected group we " "did sent the key share " "for",
                    ):
                        yield result
            ...
            if not cookie and not sr_key_share_ext:
                ...
                for result in self._sendError(
                    AlertDescription.illegal_parameter,
                    "Received HRR did not cause " "update to Client Hello",
                ):
                    yield result
        ...
        if hello_retry and hello_retry.cipher_suite != serverHello.cipher_suite:
            for result in self._sendError(
                AlertDescription.illegal_parameter,
                "server selected different cipher " "in HRR and Server Hello",
            ):
                yield result
        if real_version < settings.minVersion:
            for result in self._sendError(
                AlertDescription.protocol_version,
                "Too old version: {0} (min: {1})".format(
                    real_version, settings.minVersion
                ),
            ):
                yield result
        if real_version > settings.maxVersion and real_version not in settings.versions:
            for result in self._sendError(
                AlertDescription.protocol_version,
                "Too new version: {0} (max: {1})".format(
                    real_version, settings.maxVersion
                ),
            ):
                yield result
        ...
        if serverHello.next_protos and not clientHello.supports_npn:
            for result in self._sendError(
                ...
                yield result
        if (
            not serverHello.getExtension(ExtensionType.extended_master_secret)
        ...
        if alpnExt:
            ...
            if not clntAlpnExt:
                for result in self._sendError(
                    AlertDescription.unsupported_extension,
                    "Server sent ALPN extension without one in " "client hello",
                ):
                    yield result
        ...
        if heartbeat_ext:
            ...
            if (
                ...
                and settings.heartbeat_response_callback
            ):
                self.heartbeat_can_send = True
                self.heartbeat_response_callback = settings.heartbeat_response_callback
            elif (
                heartbeat_ext.mode == HeartbeatMode.PEER_NOT_ALLOWED_TO_SEND
                or not settings.heartbeat_response_callback
            ):
    ...
    def _clientTLS13Handshake(
        self, settings, session, clientHello, clientCertChain, privateKey, serverHello
    ):
        ...
        if sr_kex:
            ...
            cl_kex = next(
                (i for i in cl_key_share_ex.client_shares if i.group == sr_kex.group),
                None,
            )
            if cl_kex is None:
                raise TLSIllegalParameterException(
                    "Server selected not " "advertised group."
                )
        ...
        if sr_psk:
            ...
            if psk:
                ...
                psk = HandshakeHelpers.calc_res_binder_psk(
                    ident, session.resumptionMasterSecret, session.tickets
                )
        else:
        ...
        sr_handshake_traffic_secret = derive_secret(
            secret, bytearray(b"s hs traffic"), self._handshake_hash, prfName
        )
        cl_handshake_traffic_secret = derive_secret(
            secret, bytearray(b"c hs traffic"), self._handshake_hash, prfName
        )
        ...
        for result in self._getMsg(
            ContentType.handshake, HandshakeType.encrypted_extensions
        ):
            if result in (0, 1):
                yield result
            else:
        ...
        if not sr_psk:
            ...
            for result in self._getMsg(
                ContentType.handshake, expected_types, CertificateType.x509
            ):
                if result in (0, 1):
                    yield result
                else:
            ...
            if isinstance(result, CompressedCertificate):
                ...
                if (
                    compress_cert_msg.chosen_algorithm
                    not in compress_cert_ext.advertised_algorithms
                ):
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                ...
                version = serverHello.getExtension(
                ...
                try:
                    ...
                    for result in self._sendError(
                        AlertDescription.bad_certificate, formatExceptionTrace(e)
                    ):
                        yield result
                except SyntaxError as e:
                    for result in self._sendError(
                        AlertDescription.decode_error, formatExceptionTrace(e)
                    ):
                        yield result
            else:
                certificate = result
            ...
            for result in self._getMsg(
                ContentType.handshake, HandshakeType.certificate_verify
            ):
                if result in (0, 1):
                    yield result
                else:
            ...
            if not method(
                ...
                raise TLSDecryptionFailed(
                    "server Certificate Verify " "signature " "verification failed"
                )

        transcript_hash = self._handshake_hash.digest(prfName)

        for result in self._getMsg(
            ContentType.handshake, HandshakeType.finished, prf_size
        ):
            if result in (0, 1):
                yield result
            else:
        ...
        finished_key = HKDF_expand_label(
            sr_handshake_traffic_secret, b"finished", b"", prf_size, prfName
        )
        ...
        cl_app_traffic = derive_secret(
            secret, bytearray(b"c ap traffic"), server_finish_hs, prfName
        )
        sr_app_traffic = derive_secret(
            secret, bytearray(b"s ap traffic"), server_finish_hs, prfName
        )
        ...
        if certificate_request:
            ...
            if clientCertChain:
                ...
                # server requested
                if (
                    serverHello.certificate_type == CertificateType.x509
            ...
            verify_compress_cert_ext = certificate_request.getExtension(
                ExtensionType.compress_certificate
            )
            hello_compress_cert_ext = clientHello.getExtension(
                ExtensionType.compress_certificate
            )
            ...
            if verify_compress_cert_ext and hello_compress_cert_ext:
                ...
                for requested_algo in verify_compress_cert_ext.advertised_algorithms:
                    if requested_algo in hello_compress_cert_ext.advertised_algorithms:
                        client_certificate = CompressedCertificate().create(
                            requested_algo, client_certificate
                        )
            ...
            if clientCertChain and privateKey:
                ...
                availSigAlgs = self._sigHashesToList(
                    settings, privateKey, clientCertChain, version=(3, 4)
                )
                ...
                if not ver_func(
                    signature, signature_context, pad_type, hash_name, salt_len
                ):
                    for result in self._sendError(
        ...
        exporter_master_secret = derive_secret(
            secret, bytearray(b"exp master"), self._handshake_hash, prfName
        )
        ...
        cl_finished_key = HKDF_expand_label(
            cl_handshake_traffic_secret, b"finished", b"", prf_size, prfName
        )
        cl_verify_data = secureHMAC(
            cl_finished_key, self._handshake_hash.digest(prfName), prfName
        )
        ...
        resumption_master_secret = derive_secret(
            secret, bytearray(b"res master"), self._handshake_hash, prfName
        )
    ...
    def _clientResume(self, session, serverHello, clientRandom, nextProto, settings):
        if session and (
            (session.sessionID and serverHello.session_id == session.sessionID)
            or session.tls_1_0_tickets
        ):
            ...
            for result in self._sendFinished(
                session.masterSecret, session.cipherSuite, nextProto, settings=settings
            ):
                yield result
            self.sock.flush()
    ...
    def _clientKeyExchange(
        ...
        if (
            ...
            for result in self._getMsg(
                ContentType.handshake, HandshakeType.certificate, certificateType
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        if cipherSuite not in CipherSuite.certSuites:
            for result in self._getMsg(
                ContentType.handshake, HandshakeType.server_key_exchange, cipherSuite
            ):
                if result in (0, 1):
                    yield result
                else:
        ...
        for result in self._getMsg(
            ...
            (HandshakeType.certificate_request, HandshakeType.server_hello_done),
        ):
            if result in (0, 1):
                yield result
            else:
        ...
        if (
            ...
            # get the certificate
            for result in self._clientGetKeyFromChain(
            ...
            if serverKeyExchange:
                valid_sig_algs = self._sigHashesToList(
                    settings, certList=serverCertChain
                )
                try:
        ...
        if certificateRequest:
            ...
            if self.version == (3, 3) and not [
                sig
                for sig in certificateRequest.supported_signature_algs
            ...
            if clientCertChain:
                ...
                if certificateType == CertificateType.x509 and not isinstance(
        ...
        if certificateRequest and privateKey:
            valid_sig_algs = self._sigHashesToList(
                settings, privateKey, clientCertChain
            )
            try:
    ...
    def _clientFinished(
        ...
        masterSecret = self._calculate_master_secret(
            premasterSecret, cipherSuite, clientRandom, serverRandom
        )
        self._calcPendingStates(
            cipherSuite, masterSecret, clientRandom, serverRandom, cipherImplementations
        )

        # Exchange ChangeCipherSpec and Finished messages
        for result in self._sendFinished(
            masterSecret, cipherSuite, nextProto, settings=settings
        ):
            yield result
        self.sock.flush()
        self.sock.buffer_writes = False
    ...
    def _clientGetKeyFromChain(self, certificate, settings, tack_ext=None):
        ...
        if not cert_chain or cert_chain.getNumCerts() == 0:
            for result in self._sendError(
                AlertDescription.illegal_parameter,
                "Other party sent a Certificate message without " "certificates",
            ):
                yield result
    ...
    def _handshakeServerAsyncHelper(
        ...
        if (
            not cipherSuite in CipherSuite.certAllSuites
            and not cipherSuite in CipherSuite.ecdheEcdsaSuites
        ):
            tacks = None
        ...
        if (
            clientHello.getExtension(ExtensionType.record_size_limit)
            and settings.record_size_limit
        ):
            # in TLS 1.2 and earlier we can select at most 2^14B records
            extensions.append(
                RecordSizeLimitExtension().create(
        ...
        # Perform a certificate-based key exchange
            ...
            try:
                ...
                for result in self._sendError(
                    AlertDescription.handshake_failure, str(alert)
                ):
                    yield result
        ...
        # Perform anonymous Diffie Hellman key exchange
            ...
            cipherSuite in CipherSuite.anonSuites
            or cipherSuite in CipherSuite.ecdhAnonSuites
        ):
            if cipherSuite in CipherSuite.anonSuites:
                dhGroups = self._groupNamesToList(settings)
                keyExchange = ADHKeyExchange(
                    cipherSuite, clientHello, serverHello, settings.dhParams, dhGroups
                )
            else:
                acceptedCurves = self._curveNamesToList(settings)
                defaultCurve = getattr(GroupName, settings.defaultCurve)
                keyExchange = AECDHKeyExchange(
                    cipherSuite, clientHello, serverHello, acceptedCurves, defaultCurve
                )
            for result in self._serverAnonKeyExchange(
        ...
        self.session.create(
            ...
            (serverHello.tackExt is not None),
            serverName,
            encryptThenMAC=self._recordLayer._get_pending_state_etm(),
            extendedMasterSecret=self.extendedMasterSecret,
    ...
    def _derive_key_iv(nonce, user_key, settings):
        ...
        ticket_secret = derive_secret(
            secret, bytearray(b"SessionTicket secret"), None, prf_name
        )
    ...
    def _serverSendTickets(self, settings):
        ...
        for _ in range(
            settings.ticket_count
            if self.version > (3, 3)
            else int(bool(settings.ticket_count))
        ):
            # prepare the ticket
            ...
            if self.version < (3, 4):
                ...
                new_ticket.create(settings.ticketLifetime, nonce + encrypted_ticket)
                self.tls_1_0_tickets.append(encrypted_ticket)
            else:
                new_ticket = NewSessionTicket()
                new_ticket.create(
                    settings.ticketLifetime,
    ...
    def _tryDecrypt(self, settings, identity=None, ticket=None):
        ...
        for user_key in settings.ticketKeys:
            ...
            prf = (
                "sha384"
                if ticket.cipher_suite in CipherSuite.sha384PrfSuites
                else "sha256"
            )
            ...
            psk = HandshakeHelpers.calc_res_binder_psk(
                identity, ticket.master_secret, [new_sess_ticket]
            )
    ...
    def _serverTLS13Handshake(
        ...
        if share:
            ...
            for group_name in chain(
                settings.keyShares, settings.eccCurves, settings.dhGroups
            ):
                selected_group = getattr(GroupName, group_name)
                if selected_group in share_ids:
                    cl_key_share = next(
                        i for i in share.client_shares if i.group == selected_group
                    )
                    break
            else:
                for result in self._sendError(
                    AlertDescription.internal_error, "HRR did not work?!"
                ):
                    yield result
        ...
        if (
            psks
            and (
                PskKeyExchangeMode.psk_dhe_ke in psk_types.modes
                or PskKeyExchangeMode.psk_ke in psk_types.modes
            )
            ...
            for i, ident in enumerate(psks.identities):
                ...
                try:
                    ...
                    for result in self._sendError(
                        AlertDescription.illegal_parameter, str(e)
                    ):
                        yield result
                break
        ...
        if (
            psk
            and PskKeyExchangeMode.psk_dhe_ke in psk_types.modes
            and "psk_dhe_ke" in settings.psk_modes
        ) or (psk is None and privateKey):
            ...
            try:
                shared_sec = kex.calc_shared_key(
                    key_share.private, cl_key_share.key_exchange
                )
            ...
            psk is not None
            and PskKeyExchangeMode.psk_ke in psk_types.modes
            and "psk_ke" in settings.psk_modes
        ):
            shared_sec = bytearray(prf_size)
        else:
            for result in self._sendError(
        ...
        sr_handshake_traffic_secret = derive_secret(
            secret, bytearray(b"s hs traffic"), self._handshake_hash, prf_name
        )
        cl_handshake_traffic_secret = derive_secret(
            secret, bytearray(b"c hs traffic"), self._handshake_hash, prf_name
        )
        ...
        if clientHello.getExtension(ExtensionType.cookie):
            ...
            groups += [
                getattr(GroupName, i)
                for i in settings.eccCurves
                if getattr(GroupName, i) not in groups
            ]
            groups += [
                getattr(GroupName, i)
                for i in settings.dhGroups
                if getattr(GroupName, i) not in groups
            ]
            if groups:
                ext.create(groups)
        ...
        finished_key = HKDF_expand_label(
            sr_handshake_traffic_secret, b"finished", b"", prf_size, prf_name
        )
        verify_data = secureHMAC(
            finished_key, self._handshake_hash.digest(prf_name), prf_name
        )
        ...
        cl_app_traffic = derive_secret(
            secret, bytearray(b"c ap traffic"), self._handshake_hash, prf_name
        )
        sr_app_traffic = derive_secret(
            secret, bytearray(b"s ap traffic"), self._handshake_hash, prf_name
        )
        self._recordLayer.calcTLS1_3PendingState(
            serverHello.cipher_suite,
            cl_app_traffic,
        ...
        if client_cert_chain and client_cert_chain.getNumCerts():
            ...
            valid_sig_algs = self._sigHashesToList(
                settings, certList=client_cert_chain, version=(3, 4)
            )
            ...
            if not ver_func(
                ...
                for result in self._sendError(
                    AlertDescription.decrypt_error, "signature verification failed"
                ):
                    yield result
        ...
        exporter_master_secret = derive_secret(
            secret, bytearray(b"exp master"), self._handshake_hash, prf_name
        )

        # verify Finished of client
        cl_finished_key = HKDF_expand_label(
            cl_handshake_traffic_secret, b"finished", b"", prf_size, prf_name
        )
        cl_verify_data = secureHMAC(
            cl_finished_key, self._handshake_hash.digest(prf_name), prf_name
        )
        for result in self._getMsg(
            ContentType.handshake, HandshakeType.finished, prf_size
        ):
        ...
        if cl_finished.verify_data != cl_verify_data:
            for result in self._sendError(
                AlertDescription.decrypt_error, "Finished value is not valid"
            ):
                yield result
        ...
        resumption_master_secret = derive_secret(
            secret, bytearray(b"res master"), self._handshake_hash, prf_name
        )
    ...
    def _ticket_to_session(self, settings, ticket_ext):
        ...
        session.create(
            ...
            None,  # no TACK
            False,  # no TACK
            serverName=ticket.server_name.decode("utf-8") if ticket.server_name else "",
            encryptThenMAC=ticket.encrypt_then_mac,
            extendedMasterSecret=ticket.extended_master_secret,
        )
        return session

    def _serverGetClientHello(
        self,
        settings,
        private_key,
        cert_chain,
        ...
        if not clientHello.cipher_suites or not clientHello.compression_methods:
            for result in self._sendError(
                AlertDescription.decode_error, "Malformed Client Hello message"
            ):
                yield result
        ...
        if sniExt and (not sniExt.extData or not sniExt.serverNames):
            for result in self._sendError(
                AlertDescription.decode_error, "Recevived SNI extension is malformed"
            ):
                yield result
        if sniExt and sniExt.hostNames:
            ...
            if not sniExt.hostNames[0]:
                for result in self._sendError(
                    AlertDescription.decode_error, "Received SNI extension is malformed"
                ):
                    yield result
            try:
        ...
        if emsExt and emsExt.extData:
            for result in self._sendError(
                AlertDescription.decode_error,
                "Non empty payload of the Extended " "Master Secret extension",
            ):
                yield result
        ...
        if ver_ext and (3, 4) in ver_ext.versions:
            ...
            if psk:
                if not psk.identities:
                    for result in self._sendError(
                        AlertDescription.decode_error, "No identities in PSK extension"
                    ):
                        yield result
                if not psk.binders:
                    for result in self._sendError(
                        AlertDescription.decode_error, "No binders in PSK extension"
                    ):
                        yield result
                ...
                if any(not i for i in psk.binders):
                    for result in self._sendError(
                        AlertDescription.decoder_error, "Empty binder in PSK extension"
                    ):
                        yield result
                ...
                if not psk_modes:
                    for result in self._sendError(
                        AlertDescription.missing_extension,
                        "PSK extension without psk_key_exchange_modes " "extension",
                    ):
                        yield result
            ...
            if not key_exchange:
                ...
                if key_share.client_shares is None:
                    for result in self._sendError(
                        AlertDescription.decode_error, "Empty key_share extension"
                    ):
                        yield result
                ...
                mismatch = next(
                    (
                        i
                        for i in key_share.client_shares
                        if i.group not in sup_groups.groups
                    ),
                    None,
                )
                if mismatch:
                ...
                if len(set(key_share_ids)) != len(key_share_ids):
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Client sent multiple key shares for the same " "group",
                    ):
                        yield result
            ...
            if not key_exchange:
                for result in self._sendError(
                    AlertDescription.missing_extension, "Missing extension"
                ):
                    yield result
            ...
            if early_data:
                if early_data.extData:
                    for result in self._sendError(
                        AlertDescription.decode_error, "malformed early_data extension"
                    ):
                        yield result
                if not psk:
                    for result in self._sendError(
        ...
        if ver_ext:
            ...
            if not high_ver:
                for result in self._sendError(
                    AlertDescription.protocol_version,
                    "supported_versions did not include version we " "support",
                ):
                    yield result
        if high_ver:
        ...
        if sni_ext:
            ...
            if sni and sni != name:
                alert = Alert().create(
                    AlertDescription.unrecognized_name, AlertLevel.warning
                )
        ...
        if clientGroups is not None:
            ...
            if not ffGroupIntersect:
                if clientGroups and any(
                    i for i in clientGroups if i in range(256, 512)
                ):
        ...
        if size_limit_ext:
            ...
            if settings.record_size_limit:
                ...
                if version >= (3, 4):
                    ...
                    # thus the "-1)
                    self._recv_record_limit = min(
                        2**14, settings.record_size_limit - 1
                    )
                else:
        ...
        if verifierDB:
            ...
            assert False
        cipherSuites = CipherSuite.filterForVersion(
            cipherSuites, minVersion=version, maxVersion=version
        )
        ...
        if (clientHello.session_id and sessionCache) or (
            ...
            try:
                ...
                if (
                    not session
                    and (not ticket_ext or ticket_ext and not ticket_ext.ticket)
                    and sessionCache
                    and clientHello.session_id
                ):
                ...
                if clientHello.srp_username:
                    if (
                        not session.srpUsername
                        or clientHello.srp_username
                        ...
                        for result in self._sendError(
                            ...
                            yield result
                if clientHello.server_name:
                    if not session.serverName or clientHello.server_name != bytearray(
                        session.serverName, "utf-8"
                    ):
                        for result in self._sendError(
                            AlertDescription.handshake_failure
                        ):
                            yield result
                if session.encryptThenMAC and not clientHello.getExtension(
            ...
            if session:
                ...
                if session.encryptThenMAC:
                    self._recordLayer.encryptThenMAC = True
                    etm = TLSExtension().create(
                        ExtensionType.encrypt_then_mac, bytearray(0)
                    )
                    extensions.append(etm)
                if session.extendedMasterSecret:
                    ems = TLSExtension().create(
                        ExtensionType.extended_master_secret, bytearray(0)
                    )
                ...
                # Exchange ChangeCipherSpec and Finished messages
                for result in self._sendFinished(
                    session.masterSecret, session.cipherSuite, settings=settings
                ):
                    yield result
                for result in self._getFinished(
                    session.masterSecret, session.cipherSuite
                ):
                    yield result
        ...
        try:
            (
                ...
                cert_chain,
                private_key,
            ) = self._server_select_certificate(
            ...
            for result in self._sendError(
                AlertDescription.insufficient_security, str(err)
            ):
                yield result
        ...
        if version > (3, 3):
            ...
            if share:
                ...
                for selected_group in acceptable_ids:
                    ...
                    selected_group = next(
                        (i for i in acceptable_ids if i in supported_ids), None
                    )
                    if not selected_group:
                        for result in self._sendError(
                            AlertDescription.handshake_failure,
                            "No acceptable group " "advertised by client",
                        ):
                            yield result
            ...
            if hrr_ext:
                ...
                if not ext:
                    for result in self._sendError(
                        AlertDescription.missing_extension,
                        "Key share missing in " "Client Hello",
                    ):
                        yield result
                ...
                if len(ext.client_shares) != 1:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Multiple key shares in " "second Client Hello",
                    ):
                        yield result
                if ext.client_shares[0].group != selected_group:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Client key share does not " "match Hello Retry Request",
                    ):
                        yield result
                ...
                if old_ext != new_ext:
                    if old_ext is None and new_ext:
                        ...
                        clientHello1.extensions[:] = (
                            i
                            for i in clientHello1.extensions
    ...
    def _serverSRPKeyExchange(
        ...
        keyExchange = SRPKeyExchange(
            cipherSuite, clientHello, serverHello, privateKey, verifierDB
        )
        ...
        for result in self._getMsg(
            ContentType.handshake, HandshakeType.client_key_exchange, cipherSuite
        ):
            if result in (0, 1):
                yield result
            else:
        ...
        try:
            ...
            for result in self._sendError(
                AlertDescription.illegal_parameter, "Suspicious A value"
            ):
                yield result
    ...
    def _server_select_certificate(
        self, settings, client_hello, cipher_suites, cert_chain, private_key, version
    ):
        """
        ...
        client_sigalgs = client_hello.getExtension(
        ...
        alt_certs = (
            (X509CertChain(i.certificates), i.key)
            for vh in settings.virtual_hosts
            for i in vh.keys
        )
        certs = [
            (cert, key) for cert, key in chain([(cert_chain, private_key)], alt_certs)
        ]
        ...
        for cert, key in certs:
            ...
            try:
                ...
                if settings.pskConfigs and client_psks:
                    client_identities = [i.identity for i in client_psks.identities]
                    psks_prfs = [
                        i[2] if len(i) == 3 else None
                        for i in settings.pskConfigs
                        if i[0] in client_identities
                    ]
                    if psks_prfs:
                        ciphers = CipherSuite.filter_for_prfs(ciphers, psks_prfs)
                for cipher in ciphers:
                    ...
                    if (
                        ...
                        raise TLSInsufficientSecurity(
                            "FFDHE groups not acceptable and no other common " "ciphers"
                        )
                    raise TLSHandshakeFailure("No mutual ciphersuite")

                # Find a signature algorithm based on the certificate
                try:
                    sig_scheme, _, _ = self._pickServerKeyExchangeSig(
                        settings, client_hello, cert, key, version, False
                    )
                ...
                if (
                    cert
                    and cert.x509List[0].certAlg == "ecdsa"
                    and client_groups
                    and client_sigalgs
                ):
                    ...
                    if version <= (3, 3) and curve not in client_groups:
                        raise TLSHandshakeFailure(
                            ...
                            "supported by the client: {0}".format(
                    ...
                    if version >= (3, 4):
                        if GroupName.toRepr(curve) not in (
                            ...
                            raise TLSIllegalParameterException(
                                "Curve in public key is not supported " "in TLS1.3"
                            )
    ...
    def _serverCertKeyExchange(
        ...
        try:
            ...
            for result in self._sendError(
                AlertDescription.insufficient_security, str(alert)
            ):
                yield result
        ...
        if reqCert:
            ...
            certificateRequest.create(cert_types, reqCAs, valid_sig_algs)
        ...
        if reqCert:
            if self.version == (3, 0):
                ...
                if isinstance(msg, Alert):
                    ...
                    if (
                        clientCertificate.cert_chain
                        and clientCertificate.cert_chain.getNumCerts() != 0
                    ):
        ...
        if clientCertChain:
            ...
            if self.version == (3, 3):
                valid_sig_algs = self._sigHashesToList(
                    settings, certList=clientCertChain
                )
                if certificateVerify.signatureAlgorithm not in valid_sig_algs:
                    for result in self._sendError(
                        AlertDescription.illegal_parameter,
                        "Invalid signature algorithm in Certificate " "Verify",
                    ):
                        yield result
                signatureAlgorithm = certificateVerify.signatureAlgorithm
            if (
                not signatureAlgorithm
                and clientCertChain.x509List[0].certAlg == "ecdsa"
            ):
            ...
            if signatureAlgorithm and signatureAlgorithm in (
                ...
                not signatureAlgorithm
                or signatureAlgorithm[1] != SignatureAlgorithm.ecdsa
            ):
            ...
            if not ver_func(
                certificateVerify.signature, verify_bytes, padding, hash_name, salt_len
            ):
                for result in self._sendError(
                    AlertDescription.decrypt_error, "Signature failed to verify"
                ):
                    yield result
    ...
    def _serverFinished(
        ...
        # Exchange ChangeCipherSpec and Finished messages
        for result in self._getFinished(
            masterSecret, cipherSuite, expect_next_protocol=nextProtos is not None
        ):
            yield result
    ...
    def _getFinished(
        self, masterSecret, cipherSuite=None, expect_next_protocol=False, nextProto=None
    ):
        ...
        if finished.verify_data != verifyData:
            for result in self._sendError(
                AlertDescription.decrypt_error, "Finished message is incorrect"
            ):
                yield result
        ...
        try:
            ...
            if checker:
                try:
                    ...
                    alert = Alert().create(
                        AlertDescription.close_notify, AlertLevel.fatal
                    )
                    for result in self._sendMsg(alert):
                        yield result
                    raise
    ...
    def _calculate_master_secret(
        self, premaster_secret, cipher_suite, client_random, server_random
    ):
    ...
    def _pickServerKeyExchangeSig(
        ...
        for certs, key in chain([(certList, private_key)], alt_certs):
            supported = TLSConnection._sigHashesToList(
                settings, certList=certs, version=version
            )
    ...
    def _sigHashesToList(settings, privateKey=None, certList=None, version=(3, 3)):
        ...
        if not certType or certType == "ecdsa":
            for hashName in settings.ecdsaSigHashes:
                ...
                sigAlgs.append(
                    (getattr(HashAlgorithm, hashName), SignatureAlgorithm.ecdsa)
                )
        ...
        if not certType or certType == "dsa":
            for hashName in settings.dsaSigHashes:
                ...
                sigAlgs.append(
                    (getattr(HashAlgorithm, hashName), SignatureAlgorithm.dsa)
                )
                    ...
                    try:
                        ...
                        if certType != "rsa-pss":
                            sigAlgs.append(
                                getattr(
                                    SignatureScheme,
                                    "rsa_{0}_rsae_{1}".format(schemeName, hashName),
                                )
                            )
                        if certType != "rsa":
                            sigAlgs.append(
                                getattr(
                                    SignatureScheme,
                                    "rsa_{0}_pss_{1}".format(schemeName, hashName),
                                )
                            )
                        ...
                        if schemeName == "pkcs1":
                            sigAlgs.append(
                                (
                                    getattr(HashAlgorithm, hashName),
                                    SignatureAlgorithm.rsa,
                                )
                            )
                        continue
