...
class SignatureSettings(object):
    ...
    def _sanityCheckSignatureAlgs(other):
        not_allowed = [
            alg for alg in other.rsa_sig_hashes if alg not in ALL_RSA_SIGNATURE_HASHES
        ]
        if len(not_allowed) > 0:
            raise ValueError(
                "Following signature algorithms are not allowed: "
...
class SignedObject(object):
    ...
    def verify_signature(self, publicKey, settings=None):
        ...
        if self.signature[0] == 0 and numBytes(publicKey.n) + 1 == len(self.signature):
            offset = 1
        ...
        verified = publicKey.hashAndVerify(
            self.signature[offset:], self.tbs_data, hAlg=alg
        )
        if not verified:
            raise ValueError("Signature could not be verified for {0}".format(alg))
        return True
